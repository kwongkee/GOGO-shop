    ...
    function sysconf($code, $value = false)
    {
        ...
        if ($value !== false) { // 更新配置信息
            ...
            if ($ret === true) { // 更新成功 更新系统配置缓存
                \Illuminate\Support\Facades\Cache::put('sysconf_'.$code, $value, 30); // 默认缓存30分钟
            }
            return $ret;
        } else { // 获取配置信息
            // 先从缓存中读取
            $se_val = \Illuminate\Support\Facades\Cache::get('sysconf_'.$code, function () use ($code) {
                ...
                // 如果缓存中不存在 则写入缓存
                \Illuminate\Support\Facades\Cache::put('sysconf_'.$code, $configValue, 30); // 默认缓存30分钟
                return $configValue;
            });
...
if (! function_exists('shopconf')) {
    ...
    function shopconf($code, $value = false, $shop_id = 0)
    {
        ...
        if ($value !== false) {
            ...
            if ($ret === true) { // 更新成功 更新系统配置缓存
                \Illuminate\Support\Facades\Cache::put('shopconf_'.$shop_id.'_'.$code, $value, 30);
            }
            return $ret;
        } else { // 获取配置信息
            // 先从缓存中读取
            $se_val = \Illuminate\Support\Facades\Cache::get('shopconf_'.$shop_id.'_'.$code, function () use ($shop_id, $condition, $code) {
                // 缓存不存在 从数据库中读取
                ...
                // 如果缓存中不存在 则写入缓存
                \Illuminate\Support\Facades\Cache::put('shopconf_'.$shop_id.'_'.$code, $configValue, 30); // 默认缓存30分钟
                return $configValue;
            });
...
if (! function_exists('user_log')) {
    ...
    {
        // 访问客户端类型 pc、mobile
        if ($_SERVER['HTTP_HOST'] == env('MOBILE_DOMAIN')) {
