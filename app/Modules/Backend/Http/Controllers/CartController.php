...
class CartController extends Backend2
{
...
//             dd($shop_cart_list);
            foreach ($shop_cart_list as $k=>$v) {
                ...
                if (!empty($shop_cart_list[$k]['goods_info']['shop_id'])) {
                    $shop_cart_list[$k]['shop_info'] = Db::table('shop')->where(['shop_id'=>$shop_cart_list[$k]['goods_info']['shop_id']])->first();
...
//                'prefe_reduction'=>$goods_data['prefe_reduction'],
    ...
    public function cart_detail(Request $request)
    {
        ...
        #供应商START===========================
//        dd($cart_buylist['content']['goods_info']);
        $shoper = [];
...
//                    'content' => json_encode($order['content'], true),
                ...
                $collect_id = Db::connection('shop_db')->table('customs_collection')->insertGetId([
                    ...
                    'ordersn' => $ordersn,
                    'trade_price' => $settlement['true_money'],
                    'trade_type' => 3,
                    'service_info' => json_encode(['订单支付,编号：' . $new_ordersn . ',' . $settlement['true_money']], true),
                    'order_type' => 1,
                    'good_id' => '',
                    ...
                    'overdue' => '',
                    'overdue_money' => 0,
                    'total_money' => $settlement['true_money'],
                    'trans_form' => 1,
                    'status' => 0,
                    'basic' => 2,
