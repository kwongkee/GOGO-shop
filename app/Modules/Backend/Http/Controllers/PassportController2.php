...
class PassportController2 extends Controller
{
...
//        $this->username = config('admin.global.username');
    ...
    public function showLoginForm(Request $request)
    {
        ...
        $admin = '';
        if (!empty($id)) {
            $admin = Db::table('admin')->where(['user_id'=>$id])->first();
        }
        if (session('admin')) {
//            if(!empty($id)){
//                header('Location: /goods/default/list');
//            }else{
            header('Location: /');
//            }
        }
//        dd(session()->flash('user_name'));
        ...
        return view('passport.login', compact('login_bg', 'admin'));
    }
