...
class FuncController extends Frontend
{
    #去除重复字段
        ...
        if (isset($data['pa2'])) {
            ...
            if ($hotsearchId>0) {
                ...
                if ($id==1) {
                    ...
                    #1.1、获取当前导流板块关键字
                    $datas = Db::table('guide_body')->where(['id'=>$get_keywords['pid']])->first();
                    $datas = objtoarr($datas);
                    $shelf_keywords = '';
                    #1.2、获取当前导流板块的商户上架关键字
                    $goods_shelf = Db::table('goods_shelf')->where(['type'=>2,'guide_id'=>$get_keywords['pid']])->get();
...
//                                }
                ...
                #先查询有无此商品名称
                #后再查询分类名称
                $cateinfo1 = Db::table('goods')->where('goods_name', 'like', '%'.$catename.'%')->first();
                $cateinfo1 = objtoarr($cateinfo1);
                ...
                if ($cateinfo1['goods_id']>0) {
                    ...
                    $list_info = $this->getTotalWhere($catename, $g_condition, $field_condition, $condition_arr2, [['goods_name', 'like', '%'.$catename.'%'],['goods_status','=',1]], [], $sort_info, ['page'=>$page,'limit'=>$limit]);
                    ...
                    $maxprice = $list_info[3];
                    #获取原“商品名称/分类名称”的条件
                    $list2 = Db::table('goods')->where([['goods_name', 'like', '%'.$catename.'%']])->get();
                    $list2 = objtoarr($list2);
                    $condition = $this->get_condition($id, $list2, 1, ['value_show'=>0,'brand_show'=>0]);
                } else {
                    $cateinfo1 = Db::table('category')->where(['cat_name'=>$catename])->first();
                    ...
                    #获取高级条件
                    $list_info = $this->getTotalWhere($catename, $g_condition, $field_condition, $condition_arr2, [['cat_id', '=', $cateinfo1['cat_id']],['goods_status','=',1]], $sort_info, ['page'=>$page,'limit'=>$limit]);
                    $list = $list_info[0];
                    $goods_count = $list_info[1];
                    $minprice = $list_info[2];
                    $maxprice = $list_info[3];
                    ...
                    #获取原“商品名称/分类名称”的条件
                    $list2 = Db::table('goods')->where(['cat_id'=>$cateinfo1['cat_id']])->get();
                    $list2 = objtoarr($list2);
