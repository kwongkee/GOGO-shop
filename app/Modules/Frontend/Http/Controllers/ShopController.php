...
//            $list2[$k]['goods_num'] = Db::table('goods')->where(['shop_id'=>$v['shop_id']])->count();
//        }
//        $sort = array_column($list2,'goods_num');
//        array_multisort($sort,SORT_DESC,$list2);
//        $list = json_encode(,true);
        if (!$list->isEmpty()) {
            foreach ($list as $item) {
                $item->distance = $item->distance > 0 ? round($item->distance/1000, 2) : 0;
                $item->goods_num = Db::table('goods')->where(['shop_id'=>$item->shop_id])->count();
            }
            $list = objtoarr($list);
            $sort = array_column($list, 'goods_num');
