...
class SiteController extends Foundation
{
     ...
     * @param UploadVideoRepository $uploadVideoRepository
...
//        $tpl = 'partials._image_gallery_list';
        if ($request->method() == 'POST') {
            ...
            return result(0, $uploadRes['data'], '上传成功！', ['count' => $uploadRes['count'],'fuck'=>1]);
        }
...
//        dd($params);
        ...
        if (!$dir_id) {
            if (isset($video_dir_list[0]->dir_id)) {
...
//        $code = 999999;
        ...
        if (strpos($account, '@') !== false) {
            #邮箱
            $res=httpRequest('https://shop.gogo198.cn/collect_website/public/?s=/api/sendemail/index', ['email'=>$account,'title'=>'登录Gogo购购网','content'=>'验证码：'.$code.'，您正在登录Gogo购购网。']);
        } else {
            #手机
            $post_data = [
                'spid'=>'254560',
