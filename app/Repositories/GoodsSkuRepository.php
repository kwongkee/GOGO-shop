        ...
        if (!empty($data)) {
            foreach ($data as $k=>$v) {
                $item = [
                    ...
                    'sku_prices'=>json_decode($v['sku_prices'], true),
                    //+=自定义参数 END
                ];

                if (!empty($v['spec_vids'])) {
                    // 有规格

                    $spec_ids = explode('|', $v['spec_vids']); // 商品规格表主键id 12|21
                    $atrr_vids = [];
                    foreach ($spec_ids as $sid) {
//                        $atrr_vids[] = GoodsSpec::where(['goods_id'=>$v['goods_id'],['attr_vid'=>$sid]])->value('attr_vid');
                        $atrr_vids[] = $sid;
                    }
    ...
    public function getGoodsSkuInfo($skuId, $goods_info, $shop_info)
    {
        ...
        if (!empty($spec_ids)) {
            //修改bug
            $spec = Db::table('goods_spec')->where(['goods_id'=>$goods_info->goods_id,'attr_vid'=>explode('|', $info->spec_vids)[0]])->first();
            if (isset($spec)) {
                $where[] = ['spec_id', $spec->spec_id];
            } else {
                $spec = Db::table('goods_spec')->where(['goods_id'=>$goods_info->goods_id,'attr_vid'=>explode('|', $info->spec_vids)[1]])->first();
                $where[] = ['spec_id', $spec->spec_id];
            }
        } else {
            $where[] = ['spec_id', 0];
        ...
        $sku = [
            ...
            'sku_prices' => json_decode($info->sku_prices, true)
        ];
        return $sku;
    }
}
