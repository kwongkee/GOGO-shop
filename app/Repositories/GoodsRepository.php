...
//            $cat_arr = $this->categoryRep->getCatIds($goodsInsert['cat_id']);
            ...
            #最下一层销售分类
            $last_catid = 0;
            if (!empty($goodsInsert['sale_cate2'])) {
                $last_catid = $goodsInsert['sale_cate2'];
                $postData['GoodsModel']['shihe_changwei']['gcate'][0] = Db::table('category')->where(['cat_id'=>$goodsInsert['sale_cate1']])->first()->cat_name;
                $postData['GoodsModel']['shihe_changwei']['gcate'][1] = Db::table('category')->where(['cat_id'=>$goodsInsert['sale_cate2']])->first()->cat_name;
            } elseif (!empty($goodsInsert['sale_cate1'])) {
                $last_catid = $goodsInsert['sale_cate1'];
                $postData['GoodsModel']['shihe_changwei']['gcate'][0] = Db::table('category')->where(['cat_id'=>$goodsInsert['sale_cate1']])->first()->cat_name;
            }
            $goodsInsert['cat_id'] = $last_catid;
            $goodsInsert['cat_id1'] = $goodsInsert['sale_cate1'];
            $goodsInsert['cat_id2'] = isset($goodsInsert['sale_cate2']) ? $goodsInsert['sale_cate2'] : '';

            $goodsInsert['goods_video'] = $goodsInsert['goods_video2'];
...
//            $goodsInsert['other_attrs'] = json_encode(['value_name'=>$goodsInsert['value_name'],'value_desc'=>$goodsInsert['value_desc']]);  // 店铺自定义属性
            #自定义添加===START
//            $goodsInsert['crossborder_cate'] = json_encode(['crossb_cate'=>$postData['GoodsModel']['crossb_cate'],'prohibit_cate'=>$postData['GoodsModel']['prohibit_cate']],true);
            $goodsInsert['brand_type'] = $postData['GoodsModel']['brand_type'];
            $goodsInsert['brand_type2'] = $postData['GoodsModel']['brand_type2'];

            #=== 卖家说明 ===
            $user_content = [];
            foreach ($postData['parag_num2'] as $k=>$v) {
                if (isset($postData['parag_num2'][$k])) {
                    #标题判断
                    if (!empty($postData['title2'][$k])) {
                        $postData['is_title2'][$k]=1;
                    } else {
                        $postData['is_title2'][$k]=-1;
                    }
                    array_push($user_content, [
                        'parag_num'=>trim($v),
                        'pnum'=>$postData['pnum2'][$k],#上级索引
                        'is_title'=>$postData['is_title2'][$k],#是否有标题
                        'title'=>trim($postData['title2'][$k]),
                        'content'=>trim($postData['content2'][$k]),
                    ]);
                }
            }
            $user_content = json_encode($user_content, true);
            #=== 卖家说明END ===
            #优惠减免
            $goodsInsert['reduction_content'] = json_encode(['preferential_blong'=>$postData['reduction']['preferential_blong'],'type'=>$postData['reduction']['type'],'strict'=>$postData['reduction']['strict'],'currency1'=>$postData['reduction']['currency1'],'price1'=>$postData['reduction']['price1'],'currency2'=>$postData['reduction']['currency2'],'price2'=>$postData['reduction']['price2']], true);
            #优惠随赠
            $goodsInsert['gift_content'] = json_encode(['preferential_blong'=>$postData['gift']['preferential_blong'],'type'=>$postData['gift']['type'],'operaer'=>$postData['gift']['operaer'],'points_type'=>$postData['gift']['points_type'],'points_currency'=>$postData['gift']['points_currency'],'points_money'=>$postData['gift']['points_money'],'points_send'=>$postData['gift']['points_send'],'coupon_currency'=>$postData['gift']['coupon_currency'],'coupon_money'=>$postData['gift']['coupon_money'],'coupon_num'=>$postData['gift']['coupon_num'],'accgift_type'=>$postData['gift']['accgift_type'],'accgift_content'=>$postData['gift']['accgift_content'],'accgift_num'=>$postData['gift']['accgift_num'],'strict'=>$postData['gift']['strict']], true);
            #价格未含
            $goodsInsert['noinclude_content'] = json_encode(['name'=>$postData['noinclude']['name'],'desc'=>$postData['noinclude']['desc'],'currency'=>$postData['noinclude']['currency'],'price'=>$postData['noinclude']['price']], true);
            #潜在收费
            $goodsInsert['potential_content'] = json_encode(['currency'=>$postData['potential']['currency'],'name'=>$postData['potential']['name'],'desc'=>$postData['potential']['desc'],'currency2'=>$postData['potential']['currency2'],'price'=>$postData['potential']['price']], true);
            #其他费用
            $goodsInsert['otherfee_content'] = json_encode(['name'=>$postData['other_fee']['name'],'desc'=>$postData['other_fee']['desc'],'standard'=>$postData['other_fee']['standard'],'currency'=>$postData['other_fee']['currency'],'price'=>$postData['other_fee']['price']], true);
            $goodsInsert['crossborder_restrict'] = json_encode(['is_export'=>$postData['GoodsModel']['is_export'],'export_country'=>isset($postData['GoodsModel']['export_country']) ? $postData['GoodsModel']['export_country'] : '','is_gather'=>$postData['GoodsModel']['is_gather'],'is_outcurrency'=>$postData['GoodsModel']['is_outcurrency']], true);
//            $goodsInsert['platform_intro'] = $platform_content;
            $goodsInsert['userprice_intro'] = $user_content;
            $goodsInsert['have_specs'] = $postData['have_specs'];
            $goodsInsert['platform_preferent'] = json_encode(['bao_you'=>$postData['GoodsModel']['bao_you'],'full_buy_currency'=>$postData['GoodsModel']['full_buy_currency'],'full_buy_price'=>$postData['GoodsModel']['full_buy_price'],'full_buy_minusprice'=>$postData['GoodsModel']['full_buy_minusprice'],'full_buy_num'=>$postData['GoodsModel']['full_buy_num'],'full_buy_unit'=>$postData['GoodsModel']['full_buy_unit'],'full_buy_deliverynum'=>$postData['GoodsModel']['full_buy_deliverynum'],'shiwu_desc'=>$postData['GoodsModel']['shiwu_desc'],'fuwu_desc'=>$postData['GoodsModel']['fuwu_desc']], true);
            $goodsInsert['merchant_valueadd'] = json_encode(['valueadd_name'=>$postData['GoodsModel']['valueadd_name'],'valueadd_desc'=>$postData['GoodsModel']['valueadd_desc']], true);
            $goodsInsert['activity_info'] = json_encode(['have_activity'=>$postData['GoodsModel']['have_activity'],'activity_id'=>$postData['GoodsModel']['activity_id']], true);
            $goodsInsert['promotion_text'] = json_encode(['shihe_brandname'=>isset($postData['GoodsModel']['shihe_changwei']['brandname']) ? $postData['GoodsModel']['shihe_changwei']['brandname'] : '','shihe_gcate'=>$postData['GoodsModel']['shihe_changwei']['gcate'],'shihe_goodsname'=>$postData['GoodsModel']['shihe_changwei']['goodsname'],'shihe_gattr_promote'=>isset($postData['GoodsModel']['shihe_changwei']['gattr_promote']) ? $postData['GoodsModel']['shihe_changwei']['gattr_promote'] : '','shihe_goption'=>isset($postData['GoodsModel']['shihe_changwei']['goption']) ? $postData['GoodsModel']['shihe_changwei']['goption'] : '','shihe_renqun'=>isset($postData['GoodsModel']['shihe_renqun']) ? $postData['GoodsModel']['shihe_renqun'] : '','shihe_country'=>isset($postData['GoodsModel']['shihe_country']) ? $postData['GoodsModel']['shihe_country'] : '','shihe_media'=>isset($postData['GoodsModel']['shihe_media']) ? $postData['GoodsModel']['shihe_media'] : '','shihe_festival'=>isset($postData['GoodsModel']['shihe_festival']) ? $postData['GoodsModel']['shihe_festival'] : '','shihe_commongoods'=>isset($postData['GoodsModel']['shihe_commongoods']) ? $postData['GoodsModel']['shihe_commongoods'] : '','shihe_zongjiao'=>isset($postData['GoodsModel']['shihe_zongjiao']) ? $postData['GoodsModel']['shihe_zongjiao'] : ''], true);
            if ($postData['have_specs']==2) {
                $goodsInsert['nospecs'] = json_encode($postData['nospecs'], true);
            }
            #自定义添加===END
            ...
            #=== 商品品牌 ===
            if ($postData['GoodsModel']['brand_type']==2 && $postData['GoodsModel']['brand_id']==-1 && !empty($postData['diy_brandname'])) {
                $ishave = Db::table('brand')->where(['brand_name'=>trim($postData['diy_brandname'])])->first();
                if ($ishave->id) {
                    $postData['brand_id'] = $ishave->id;
                } else {
                    $postData['brand_id'] = Db::table('brand')->insertGetId(['brand_name'=>trim($postData['diy_brandname']),'typeid'=>$last_catid,'created_at'=>date('Y-m-d H:i:s')]);
                    #顺便调整分类下的商品字段
                    $cate = Db::table('category')->where(['cat_id'=>$last_catid])->first();
                    $cate = $this->common->objectToArrays($cate);
                    $brand_ids = $cate['brand_ids'].$postData['GoodsModel']['brand_id'].',';
                    Db::table('category')->where(['cat_id'=>$last_catid])->update(['brand_ids'=>$brand_ids]);
                }
            }
            #=== 商品品牌END ===
            ...
            if (isset($postData['sale_cate'])) {
                if (!empty($postData['othersale_attr_name'])) {
                    foreach ($postData['othersale_attr_name'] as $k => $v) {
                        if (!empty($v)) {
                            $pid = Db::table('ssl_value')->insertGetId(['good_category'=>1,'cate_id'=>$last_catid,'pid'=>0,'name'=>trim($v)]);
                            $postData['sale_cate']['value_name'][$k] = $pid;
                            $other_attr_value = explode(',', $postData['othersale_attr_value'][$k]);
                            foreach ($other_attr_value as $k2=>$v2) {
                                if (!empty($v2)) {
                                    $vid = Db::table('ssl_value')->insertGetId(['pid'=>$pid,'name'=>trim($v2)]);
                                    $postData['sale_cate']['value_desc'][$k] = $vid;
                                }
                            }
                        }
                    }
            ...
            #=== 跨境物流属性 ===
            $last_catid2 = 0;
            if (!empty($goodsInsert['crossb_cate2'])) {
                $last_catid2 = $goodsInsert['crossb_cate2'];
            } elseif (!empty($goodsInsert['crossb_cate1'])) {
                $last_catid2 = $goodsInsert['crossb_cate1'];
            }
            if (!empty($postData['otherlogi_attr_name'])) {
                foreach ($postData['otherlogi_attr_name'] as $k => $v) {
                    if (!empty($v)) {
                        $pid = Db::table('ssl_value')->insertGetId(['good_category'=>2,'cate_id'=>$last_catid2,'pid'=>0,'name'=>trim($v)]);
                        $postData['logi_cate']['value_name'][$k] = $pid;
                        $other_attr_value = explode(',', $postData['otherlogi_attr_value'][$k]);
                        foreach ($other_attr_value as $k2=>$v2) {
                            if (!empty($v2)) {
                                $vid = Db::table('ssl_value')->insertGetId(['pid'=>$pid,'name'=>trim($v2)]);
                                $postData['logi_cate']['value_desc'][$k] = $vid;
                            }
                        }
                    }
                }
            ...
            if (isset($postData['sale_cate'])) {
                Db::table('goods')->where(['goods_id' => $goodsRet->goods_id])->update([
                    'other_attrs' => serialize([
                        'value_name' . $last_catid => $postData['sale_cate']['value_name'], 'value_desc' . $last_catid => $postData['sale_cate']['value_desc'],
                        'value_name' . $last_catid2 => $postData['logi_cate']['value_name'], 'value_desc' . $last_catid2 => $postData['logi_cate']['value_desc'],
                    ])
                ]);
            }
            #=== 商品属性END ===
            ...
            if ($postData['have_specs']==2) {
                // 如果没有设置规格 则新增一条商品SKU信息
                $goodsInsert['nospecs'] = json_decode($goodsInsert['nospecs'], true);
                $goodsSkuInsert = [
                    'goods_id' => $goodsRet->goods_id,
                    'market_price' => isset($goodsInsert['market_price']) ? $goodsInsert['market_price'] : 0,
                    'goods_price' => isset($goodsInsert['goods_price']) ? $goodsInsert['goods_price'] : 0,
                    'goods_number' => isset($goodsInsert['nospecs']['goods_number'][0]) ? $postData['nospecs']['goods_number'][0] : 0,
                    'warn_number' => isset($goodsInsert['warn_number']) ? $goodsInsert['warn_number'] : 0,
                    'goods_sn' => isset($goodsInsert['goods_sn']) ? $goodsInsert['goods_sn'] : '',
                    'goods_barcode' => isset($goodsInsert['goods_barcode']) ? $goodsInsert['goods_barcode'] : '',
                    'goods_stockcode' => isset($goodsInsert['goods_stockcode']) ? $goodsInsert['goods_stockcode'] : '',
                    'is_spu' => 1, // 无规格商品 是SPU商品
                    'sku_prices' => json_encode([
                        'goods_number'=>isset($postData['nospecs']['goods_number'][0]) ? $postData['nospecs']['goods_number'][0] : 0,
                        'start_num'=>$postData['nospecs']['start_num'],
                        'unit'=>$postData['nospecs']['unit'],
                        'select_end'=>$postData['nospecs']['select_end'],
                        'end_num'=>$postData['nospecs']['end_num'],
                        'currency'=>$postData['nospecs']['currency'],
                        'price'=>$postData['nospecs']['price'],
                    ], true)#该规格的区间价格
                ];
                $goodsSku = new GoodsSku();
                $goodsSku->fill($goodsSkuInsert);
                ...
                // 如果设置了规格 在商品SKU表中新增所有规格的SKU信息
                #清空第0个goods_number,因为第0个为无规格的库存
                $sku_number = [];
                foreach ($postData['new_specs']['goods_number'] as $k=>$v) {
                    if (!empty($v)) {
                        $sku_number[$k] = $v;
                    }
                }
                $postData['new_specs']['goods_number'] = array_values($sku_number);

                #整理规格===START
                $big_specs = [];
                $fspecs = '';
                foreach ($postData['goods_specs'] as $k1=>$v1) {
                    if ($k1==0) {
                        $fspecs = $v1['attr_id'];
                    }
                    if (!in_array($v1['attr_id'], $big_specs)) {
                        array_push($big_specs, $v1['attr_id']);
                    }
                }
                $real_specs = [];
                foreach ($big_specs as $k1=>$v1) {
                    $real_specs[$v1] = [];
                    foreach ($postData['goods_specs'] as $k2=>$v2) {
                        if ($v1 == $v2['attr_id']) {
                            $real_specs[$v2['attr_id']] = array_merge($real_specs[$v2['attr_id']], [$v2['attr_vid']]);
                        }
                    }
                }
                ...
                $first_specs = [];
                $after_specs = [];
                foreach ($real_specs as $k1=>$v1) {
                    if ($fspecs != $k1) {
                        array_push($after_specs, $v1);
                    } else {
                        $first_specs = array_merge($first_specs, $real_specs[$fspecs]);
                    }
                }

                $insert_specs = [];
                foreach ($first_specs as $k1=>$v1) {
                    if (!empty($after_specs)) {
                        foreach ($after_specs as $k2=>$v2) {
                            foreach ($after_specs[0] as $k3=>$v3) {
                                if (count($after_specs)==1) {
                                    if (!in_array($v1.'*'.$v3, $insert_specs)) {
                                        array_push($insert_specs, $v1 . '*' . $v3);
                                    }
                                } elseif (count($after_specs)==2) {
                                    foreach ($after_specs[1] as $k4=>$v4) {
                                        if (!in_array($v1.'*'.$v3.'*'.$v4, $insert_specs)) {
                                            array_push($insert_specs, $v1 . '*' . $v3 . '*' . $v4);
                                        }
                                    }
                                } elseif (count($after_specs)==3) {
                                    foreach ($after_specs[1] as $k4=>$v4) {
                                        foreach ($after_specs[2] as $k5=>$v5) {
                                            if (!in_array($v1.'*'.$v3.'*'.$v4.'*'.$v5, $insert_specs)) {
                                                array_push($insert_specs, $v1 . '*' . $v3 . '*' . $v4 . '*' . $v5);
                                            }
                                        }
                                    }
                                } elseif (count($after_specs)==4) {
                                    foreach ($after_specs[1] as $k4=>$v4) {
                                        foreach ($after_specs[2] as $k5=>$v5) {
                                            foreach ($after_specs[3] as $k6=>$v6) {
                                                if (!in_array($v1.'*'.$v3.'*'.$v4.'*'.$v5.'*'.$v6, $insert_specs)) {
                                                    array_push($insert_specs, $v1 . '*' . $v3 . '*' . $v4 . '*' . $v5 . '*' . $v6);
                                                }
                                            }
                                        }
                                    }
                                } elseif (count($after_specs)==5) {
                                    foreach ($after_specs[1] as $k4=>$v4) {
                                        foreach ($after_specs[2] as $k5=>$v5) {
                                            foreach ($after_specs[3] as $k6=>$v6) {
                                                foreach ($after_specs[4] as $k7=>$v7) {
                                                    if (!in_array($v1.'*'.$v3.'*'.$v4.'*'.$v5.'*'.$v6.'*'.$v7, $insert_specs)) {
                                                        array_push($insert_specs, $v1 . '*' . $v3 . '*' . $v4 . '*' . $v5 . '*' . $v6 . '*' . $v7);
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } elseif (count($after_specs)==6) {
                                    #7层规格
                                    foreach ($after_specs[1] as $k4=>$v4) {
                                        foreach ($after_specs[2] as $k5=>$v5) {
                                            foreach ($after_specs[3] as $k6=>$v6) {
                                                foreach ($after_specs[4] as $k7=>$v7) {
                                                    foreach ($after_specs[5] as $k8=>$v8) {
                                                        if (!in_array($v1.'*'.$v3.'*'.$v4.'*'.$v5.'*'.$v6.'*'.$v7.'*'.$v8, $insert_specs)) {
                                                            array_push($insert_specs, $v1.'*'.$v3.'*'.$v4.'*'.$v5.'*'.$v6.'*'.$v7.'*'.$v8);
                                                        }
                                                    }
                                                }
                                            }
                        ...
                        array_push($insert_specs, $v1);
                    }
                }

                #插入数据表
                foreach ($insert_specs as $k1=>$v1) {
                    #每个规格一条数据
                    $item['goods_id'] = $goodsRet->goods_id;

                    $attr_vids = explode('*', $v1);
                    $attr_vname = [];
                    foreach ($attr_vids as $k2=>$v2) {
                        $attr_vname2 = AttrValue::where('attr_vid', $v2)->first();
                        $attr_name2 = Attribute::where('attr_id', $attr_vname2->attr_id)->first();
                        $attr_vname = array_merge($attr_vname, [$attr_name2->attr_name.':'.$attr_vname2->attr_vname]);
                    }


                    $item['spec_ids'] = implode('|', $big_specs);#大规格
                    $item['spec_vids'] = implode('|', $attr_vids);#子规格
                    $item['spec_names'] = implode(' ', $attr_vname);#规格名称：规格属性名称

                    $item['sku_specs'] = $v1;
                    $item['goods_number'] = isset($postData['new_specs']['goods_number'][$k1]) ? $postData['new_specs']['goods_number'][$k1] : 0;

                    $item['sku_prices'] = json_encode([
                        'goods_number'=>isset($postData['new_specs']['goods_number'][$k1]) ? $postData['new_specs']['goods_number'][$k1] : 0,
                        'disabled_num'=>$postData['new_specs']['disabled_num'][$k1],
                        'start_num'=>$postData['new_specs']['start_num'][$k1],
                        'unit'=>$postData['new_specs']['unit'][$k1],
                        'select_end'=>$postData['new_specs']['select_end'][$k1],
                        'end_num'=>$postData['new_specs']['end_num'][$k1],
                        'currency'=>$postData['new_specs']['currency'][$k1],
                        'price'=>$postData['new_specs']['price'][$k1],
                    ], true);#该规格的区间价格
                    $item['is_spu'] = 0; // 商品有规格 不是SPU商品
                    $goodsSku = new GoodsSku();
                    $goodsSku->fill($item);
...
//            $goodsUpdate['other_cat_ids'] = implode(',',$postData['other_cat_ids']); // todo
            ...
            $goodsUpdate['goods_video'] = $goodsUpdate['goods_video2'];
//            $goodsUpdate['other_attrs'] = serialize($postData['other_attrs']);  // 店铺自定义属性
            ...
            #自定义修改===START
            $goodsUpdate['crossborder_cate'] = json_encode(['crossb_cate'=>$postData['GoodsModel']['crossb_cate'],'prohibit_cate'=>$postData['GoodsModel']['prohibit_cate']], true);
            $goodsUpdate['brand_type'] = $postData['GoodsModel']['brand_type'];
            $goodsUpdate['brand_type2'] = $postData['GoodsModel']['brand_type2'];

            #=== 平台价格说明 ===
            $platform_content = [];
            foreach ($postData['parag_num1'] as $k=>$v) {
                if (isset($postData['parag_num1'][$k])) {
                    #标题判断
                    if (!empty($postData['title1'][$k])) {
                        $postData['is_title1'][$k]=1;
                    } else {
                        $postData['is_title1'][$k]=-1;
                    }
                    array_push($platform_content, [
                        'parag_num'=>trim($v),
                        'pnum'=>$postData['pnum1'][$k],#上级索引
                        'is_title'=>$postData['is_title1'][$k],#是否有标题
                        'title'=>trim($postData['title1'][$k]),
                        'content'=>trim($postData['content1'][$k]),
                    ]);
                }
            }
            $platform_content = json_encode($platform_content, true);
            #=== 平台价格说明END ===
            ...
            $goodsUpdate['crossborder_restrict'] = json_encode(['is_export'=>$postData['GoodsModel']['is_export'],'export_country'=>isset($postData['GoodsModel']['export_country']) ? $postData['GoodsModel']['export_country'] : '','is_gather'=>$postData['GoodsModel']['is_gather'],'is_outcurrency'=>$postData['GoodsModel']['is_outcurrency']], true);
            $goodsUpdate['platform_intro'] = $platform_content;
            $goodsUpdate['userprice_intro'] = $user_content;
            $goodsUpdate['have_specs'] = $postData['have_specs'];
            $goodsUpdate['platform_preferent'] = json_encode(['bao_you'=>$postData['GoodsModel']['bao_you'],'full_buy_currency'=>$postData['GoodsModel']['full_buy_currency'],'full_buy_price'=>$postData['GoodsModel']['full_buy_price'],'full_buy_minusprice'=>$postData['GoodsModel']['full_buy_minusprice'],'full_buy_num'=>$postData['GoodsModel']['full_buy_num'],'full_buy_unit'=>$postData['GoodsModel']['full_buy_unit'],'full_buy_deliverynum'=>$postData['GoodsModel']['full_buy_deliverynum'],'shiwu_desc'=>$postData['GoodsModel']['shiwu_desc'],'fuwu_desc'=>$postData['GoodsModel']['fuwu_desc']], true);
            $goodsUpdate['merchant_valueadd'] = json_encode(['valueadd_name'=>$postData['GoodsModel']['valueadd_name'],'valueadd_desc'=>$postData['GoodsModel']['valueadd_desc']], true);
            $goodsUpdate['activity_info'] = json_encode(['have_activity'=>$postData['GoodsModel']['have_activity'],'activity_id'=>$postData['GoodsModel']['activity_id']], true);
            $goodsUpdate['promotion_text'] = json_encode(['shihe_brandname'=>$postData['GoodsModel']['shihe_changwei']['brandname'],'shihe_gcate'=>$postData['GoodsModel']['shihe_changwei']['gcate'],'shihe_goodsname'=>$postData['GoodsModel']['shihe_changwei']['goodsname'],'shihe_gattr_promote'=>isset($postData['GoodsModel']['shihe_changwei']['gattr_promote']) ? $postData['GoodsModel']['shihe_changwei']['gattr_promote'] : '','shihe_goption'=>isset($postData['GoodsModel']['shihe_changwei']['goption']) ? $postData['GoodsModel']['shihe_changwei']['goption'] : '','shihe_renqun'=>isset($postData['GoodsModel']['shihe_renqun']) ? $postData['GoodsModel']['shihe_renqun'] : '','shihe_country'=>isset($postData['GoodsModel']['shihe_country']) ? $postData['GoodsModel']['shihe_country'] : '','shihe_media'=>isset($postData['GoodsModel']['shihe_media']) ? $postData['GoodsModel']['shihe_media'] : '','shihe_festival'=>isset($postData['GoodsModel']['shihe_festival']) ? $postData['GoodsModel']['shihe_festival'] : '','shihe_commongoods'=>isset($postData['GoodsModel']['shihe_commongoods']) ? $postData['GoodsModel']['shihe_commongoods'] : '','shihe_zongjiao'=>isset($postData['GoodsModel']['shihe_zongjiao']) ? $postData['GoodsModel']['shihe_zongjiao'] : ''], true);
            if ($postData['have_specs']==2) {
                $goodsUpdate['nospecs'] = json_encode($postData['nospecs'], true);
            }
            #自定义修改===END
            ...
            if ($postData['GoodsModel']['brand_type']==2 && $postData['GoodsModel']['brand_id']==-1 && !empty($postData['diy_brandname'])) {
                ...
                if ($ishave->id) {
                    ...
                    $postData['brand_id'] = Db::table('brand')->insertGetId(['brand_name'=>trim($postData['diy_brandname']),'typeid'=>$goodsUpdate['cat_id'],'created_at'=>date('Y-m-d H:i:s')]);
                    #顺便调整分类下的商品字段
                    $cate = Db::table('category')->where(['cat_id'=>$goodsUpdate['cat_id']])->first();
                    $cate = $this->common->objectToArrays($cate);
                    $brand_ids = $cate['brand_ids'].$postData['GoodsModel']['brand_id'].',';
                    Db::table('category')->where(['cat_id'=>$goodsUpdate['cat_id']])->update(['brand_ids'=>$brand_ids]);
                }
            }
            #=== 商品品牌END ===

            #=== 商品属性 ===
            if (!empty($postData['other_attr_name'])) {
                foreach ($postData['other_attr_name'] as $k => $v) {
                    if (!empty($v)) {
                        $pid = Db::table('ssl_value')->insertGetId(['pid'=>0,'name'=>trim($v)]);
                        $goodsUpdate['value_name'][$k] = $pid;
                        $other_attr_value = explode(',', $postData['other_attr_value'][$k]);
                        foreach ($other_attr_value as $k2=>$v2) {
                            if (!empty($v2)) {
                                $vid = Db::table('ssl_value')->insertGetId(['pid'=>$pid,'name'=>trim($v2)]);
                                $goodsUpdate['value_desc'][$k] = $vid;
                            }
                        }
                    }
                }
            ...
            Db::table('goods')->where(['goods_id'=>$goods_id])->update(['other_attrs'=>serialize(['value_name'=>$goodsUpdate['value_name'],'value_desc'=>$goodsUpdate['value_desc']])]);
            #####2024-01-18 end#####
            ...
            #正式规格录入
            if ($postData['have_specs']==2) {
                // 如果没有设置规格 则修改原商品的SKU信息
                $goodsUpdate['nospecs'] = json_decode($goodsUpdate['nospecs'], true);

                $goodsSkuUpdate = [
                    'market_price' => isset($goodsUpdate['market_price']) ? $goodsUpdate['market_price'] : 0,
                    'goods_price' => isset($goodsUpdate['goods_price']) ? $goodsUpdate['goods_price'] : 0,
                    'goods_number' => isset($goodsUpdate['nospecs']['goods_number'][0]) ? $goodsUpdate['nospecs']['goods_number'][0] : 0,
                    'warn_number' => isset($goodsUpdate['warn_number']) ? $goodsUpdate['warn_number'] : 0,
                    'goods_sn' => isset($goodsUpdate['goods_sn']) ? $goodsUpdate['goods_sn'] : '',
                    'goods_barcode' => isset($goodsUpdate['goods_barcode']) ? $goodsUpdate['goods_barcode'] : '',
                    'goods_stockcode' => isset($goodsUpdate['goods_stockcode']) ? $goodsUpdate['goods_stockcode'] : '',
                    'sku_prices' => json_encode([
                        'goods_number'=>isset($postData['nospecs']['goods_number'][0]) ? $postData['nospecs']['goods_number'][0] : 0,
                        'start_num'=>$postData['nospecs']['start_num'],
                        'unit'=>$postData['nospecs']['unit'],
                ...
                GoodsSku::where('goods_id', $goods_id)->update($goodsSkuUpdate);
            } elseif ($postData['have_specs']==1) {
                // 如果设置了规格 在商品SKU表中新增所有规格的SKU信息
                #清空第0个goods_number,因为第0个为无规格的库存
                $sku_number = [];
                foreach ($postData['new_specs']['goods_number'] as $k=>$v) {
                ...
                foreach ($insert_specs as $k1=>$v1) {
                    ...
                    $item2['spec_ids'] = implode('|', $big_specs);#大规格
                    $item2['spec_vids'] = implode('|', $attr_vids);#子规格
                    $item2['spec_names'] = implode(' ', $attr_vname);#规格名称：规格属性名称

                    $item2['sku_specs'] = $v1;
                    $item2['goods_number'] = isset($postData['new_specs']['goods_number'][$k1]) ? $postData['new_specs']['goods_number'][$k1] : 0;

                    $item2['sku_prices'] = json_encode([
                        'goods_number'=>isset($postData['new_specs']['goods_number'][$k1]) ? $postData['new_specs']['goods_number'][$k1] : 0,
                        'disabled_num'=>$postData['new_specs']['disabled_num'][$k1],
                        'start_num'=>$postData['new_specs']['start_num'][$k1],
                        'unit'=>$postData['new_specs']['unit'][$k1],
                    ...
                    // 检查该规格是否存在 如果存在则更新 否则新增
                    $goodsSkuCondition = [['goods_id', $goods_id], ['spec_ids', $item2['spec_ids']],['spec_vids', $item2['spec_vids']]];
                    $isExist = GoodsSku::where($goodsSkuCondition)->get()->toArray();
                    if (count($isExist) > 0) {
                        // 已经存在 更新
                        GoodsSku::where($goodsSkuCondition)->update($item2);
                    } else {
                        // 不存在 新增
                        $goodsSku = new GoodsSku();
                        $goodsSku->fill($item2);
...
//            echo $e->getCode();
    ...
    public function getGoodsModelInfo($goods_id)
    {
        $field = ['mobile_price','cost_price','market_price','invoice_type','is_repair','user_discount','stock_mode','top_layout_id','bottom_layout_id','warn_number','goods_number','goods_sort','freight_id','pricing_mode','sales_model','goods_status','goods_id','goods_name','cat_id','cat_id1','cat_id2','cat_id3','shop_id','sku_open','sku_id','goods_subname','goods_price','give_integral','goods_sn','goods_barcode','goods_image',
            ...
            'shop_cat_ids',
            'crossborder_cate','brand_type','brand_type2','have_specs','nospecs','crossborder_restrict','platform_intro','userprice_intro','platform_preferent','merchant_valueadd','activity_info','promotion_text','crossb_cate1','crossb_cate2','crossb_cate3','logistics_cate'
            ];
    ...
    public function getTopGoods($sort = 'sale_num', $cat_id_arr = [], $num = 4, $shop_id = 0)
    {
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                $low_price = '';
                foreach ($sku_info->sku_prices['price'] as $k=>$v) {
                    if (empty($low_price)) {
                        $low_price = $v;
                    } else {
                        if ($v<$low_price) {
    ...
    public function getGuessLikeGoods($page = 1, $num = 6)
    {
        ...
        $condition = [
            'where' => $where,
            'field' => ['goods_id','goods_name','goods_image','goods_price','comment_num','shop_id','sku_id'],
            'cur_page' => $page,
            'page_size' => $num,
    ...
    public function getGoodsSpecList($goods_info)
    {
        ...
        #调整多规格顺序 START===========================================================20240604
        if ($goods_info->shop_id==0) {
            #其他商城只有两级规格型号
            $spec_name = Db::table('goods_sku')->where(['goods_id'=>$goods_id])->first();
            $spec_name = objtoarr($spec_name);

            if (!empty($spec_name['spec_ids'])) {
                $spec_name['spec_ids'] = explode('|', $spec_name['spec_ids']);
                $data2 = [];
                foreach ($spec_name['spec_ids'] as $k=>$v) {
                    $data2[$v] = [];
                    $data2[$v] = array_merge($data2[$v], $data[$v]);
                }
                $data = $data2;
            }
...
//                  'spec_id'=>21,
//                  'children'=>[
//                              ['attr_id'=>1892,'have_child'=>[1,2,3]],
//                              ['attr_id'=>1893,'have_child'=>[]],
//                              ...
//                 ]
//            ]
//            ...
                    ...
                    foreach ($spec_name as $k=>$v) {
                        ...
                        foreach ($spec_name[$k]['spec_vids'] as $k2=>$v2) {
                            ...
                            if (count($spec_name[$k]['spec_vids'][$k2])>1) {
                                if (empty($data2[$k2]['children'])) {
                                    array_push($data2[$k2]['children'], ['attr_id'=>$spec_name[$k]['spec_vids'][$k2][0],'have_child'=>[$spec_name[$k]['spec_vids'][$k2][1]]]);
                                } else {
//                                    if($k>1){
//                                        dd($data2[$k2]['children']);
//                                    }
                                    $found = false;
                                    foreach ($data2[$k2]['children'] as $k3=>$v3) {
                                        if ($v3['attr_id']==$spec_name[$k]['spec_vids'][$k2][0]) {
                                            if (!in_array($spec_name[$k]['spec_vids'][$k2][1], $data2[$k2]['children'][$k3]['have_child'])) {
                                                array_push($data2[$k2]['children'][$k3]['have_child'], $spec_name[$k]['spec_vids'][$k2][1]);
                                            }
                                            $found = true;
                                        }
                                    }
                                ...
                                if (!in_array(['attr_id'=>$v2,'have_child'=>[]], $data2[$k2]['children'])) {
                                    array_push($data2[$k2]['children'], ['attr_id'=>$v2,'have_child'=>[]]);
                                }
                            }
                        }
                    }
                        ...
                        foreach ($v['children'] as $k2=>$v2) {
                            ...
                            if (!empty($v2['have_child'])) {
                                foreach ($v2['have_child'] as $k3=>$v3) {
                                    $attr_value2 = Db::table('attr_value')->where(['attr_vid'=>$v3])->first();
                                    $spec_id2 = Db::table('goods_spec')->where(['goods_id'=>$goods_info['goods_id'],'attr_id'=>$v['spec_id'],'attr_vid'=>$attr_value2->attr_vid])->select(['spec_id'])->first()->spec_id;
                                    $attr_value_children[] = [
                                        'spec_id'=>$spec_id2,
                                        'spec_image'=>'',
                                        'attr_vid'=>$attr_value2->attr_vid,
                                        'attr_value'=>$attr_value2->attr_vname,
                                        'attr_desc'=>''
                                    ];
                                }
                            }
                            $attr_values[] = [
                                ...
                                'attr_desc'=>'',
                                'children'=>$attr_value_children
                            ];
                        }
                        $attr_name = Db::table('attribute')->where(['attr_id'=>$v['spec_id']])->select(['attr_name'])->first()->attr_name;
                        $resData[] = [
                            'cat_id' => $cat_id,
                            'attr_id' => $v['spec_id'],
                            'attr_sort' => $k,
