...
//            $relateCatRight = Category::where('is_show', 1)->whereIn('parent_id', $relateCatIds)->select(['cat_id','cat_name'])->limit(12)->get();
            $relateCatRight = Category::where('parent_id', $item->cat_id)->where('is_show', 1)->get();
            if (!empty($relateCatRight)) {
                foreach ($relateCatRight as $v) {
                    $child = Category::where('is_show', 1)->where('parent_id', $v->cat_id)->select(['cat_id','cat_name'])->get();
                    $v->child = $child;
//                    if($v->cat_id==4602){
//                        foreach($v->child as $v2){
//                            $child = Category::where('is_show', 1)->where('parent_id', $v2->cat_id)->select(['cat_id','cat_name'])->get();
//                            if(!empty($child)){
//                                $v2->child = $child;
//                                foreach($v2->child as $v3){
//                                    $child = Category::where('is_show', 1)->where('parent_id', $v3->cat_id)->select(['cat_id','cat_name'])->get();
//                                    if(!empty($child)){
//                                        $v3->child = $child;
//                                        foreach($v3->child as $v4){
//                                            $child = Category::where('is_show', 1)->where('parent_id', $v4->cat_id)->select(['cat_id','cat_name'])->get();
//                                            if(!empty($child)) {
//                                                $v4->child = $child;
//                                            }else{
//                                                $v4->child = '';
