<style>
    /*解决弹框问题===start*/
    /*.confirm-order{z-index: 19991015 !important;}*/
    .layer_frame{top: 50% !important;left: 50% !important;transform: translate(-50%, -50%) !important;/*z-index: 919991014 !important;*/background:#fff;}
    .layer_frame .layui-tab-content{padding:0;}
    ...
    .gather_rule{width: 100%;margin-bottom:0px;max-width: 320px;}
    .rule_content .cursor{cursor:pointer;color:#000;text-decoration:underline;font-size:15px;max-width: 230px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;}
    .rule_content.cursor:hover{color:#000;}
    ...
    .rule_content{border:0px solid {{$website['color']}};border-radius: 5px;}
    .rule_item:first-child{margin-top:0;}
    .rule_item{margin-top:10px;color:#000;max-width: 380px;/*overflow-x: auto;white-space:nowrap;*/}
    .layui-form-item .layui-input-inline{margin-right:0;min-width: 230px;}
    .rule_content .layui-form-item:last-child{margin-bottom:0;}
    .rule_content .sure_txt{padding-left: 5px;}

    .change_window .layui-form-item .layui-form-checkbox[lay-skin=primary]{margin-top:0;}
    .change_window .layui-tab-card>.layui-tab-title{background:{{$website['color']}};}
    .change_window .layui-tab-card>.layui-tab-title li{color:{{$website['color_word']}};}
    .change_window .layui-tab-card>.layui-tab-title .layui-this{background:unset;}
    /*新增tab-card样式*/
    ...
    .change_window .layui-tab-card>.layui-tab-title li:last-child{margin-right: 0;font-size:15px;}
    ...
    .change_window .already_read:after{content: '';position: absolute;top: 42px;left: 50%;width: 15px;height: 8px;border: 2px solid #FFFFFF;border-width: 2px !important;border-top: 0;border-right:0;transform: rotate(-45deg);border-radius:0 !important;box-sizing:unset !important;}
    ...
    .rule_code{text-align: center;background:#fff;}
    ...
    @media (max-width: 992px){
        .gather_rule{width: 100%;max-width: 100%;}
    }
</style>
@if(!empty($check_content))
    ...
    <div class="gather_rule">
        ...
        <div class="rule_content" @if($check_content['already_read']==1) style="border:2px solid {{$website['color_word']}};" @endif>
        @if(!empty($check_content['confirm']['title'][0]))
        <div class="layui-form-item">
            <label class="layui-form-label"><input type="checkbox" name="confirm" title="" value="1" lay-skin="primary" lay-filter="confirm" lay-verify="required" disabled @if($check_content['already_read']==1) checked @endif><span class="sure_txt">同意</span></label>
            <div class="layui-input-inline layui-select-fscon" style="/*width: 65% !important;overflow: auto;*/">
                <div class="rule_item disf">
                    @foreach($check_content['confirm']['title'] as $key2 => $vo)
                    <div class="cursor" onclick="open_check('{{$vo}}','{{$check_content['confirm']['link'][$key2]}}',{{$check_content['confirm']['method']}},{{$check_content['confirm']['second']}},1)">《{{$vo}}》</div>@if(isset($check_content['confirm']['title'][$key2+1]))) &nbsp; @endif
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label"><input type="checkbox" name="sure" title="" value="1" lay-skin="primary" lay-filter="sure" lay-verify="required" disabled @if($check_content['already_read']==1) checked @endif><span class="sure_txt">确认</span></label>
            <div class="layui-input-inline layui-select-fscon" style="/*width: 65% !important;overflow: auto;*/">
                <div class="rule_item disf">
                    @foreach($check_content['sure']['title'] as $key2 => $vo)
                    <div class="cursor" onclick="open_check('{{$vo}}','{{$check_content['sure']['link'][$key2]}}',{{$check_content['sure']['method']}},{{$check_content['sure']['second']}},2)">《{{$vo}}》</div>@if(isset($check_content['sure']['title'][$key2+1]))) &nbsp; @endif
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label"><input type="checkbox" name="knows" title="" value="1" lay-skin="primary" lay-filter="knows" lay-verify="required" disabled @if($check_content['already_read']==1) checked @endif><span class="sure_txt">知悉</span></label>
            <div class="layui-input-inline layui-select-fscon" style="/*width: 65% !important;overflow: auto;*/">
                <div class="rule_item disf">
                    @foreach($check_content['knows']['title'] as $key2 => $vo)
                    <div class="cursor" onclick="open_check('{{$vo}}','{{$check_content['knows']['link'][$key2]}}',{{$check_content['knows']['method']}},{{$check_content['knows']['second']}},3)">《{{$vo}}》</div>@if(isset($check_content['knows']['title'][$key2+1]))) &nbsp; @endif
                    @endforeach
                </div>
            </div>
        </div>
        @endif
        <div class="layui-form-item layui-layout-admin rule_submit norepeat_submit" style="margin-bottom: 10px !important;display: none;">
    ...
    <div class="check_content" style="display: none;text-align: center;">
        ...
        <div class="out_closeBtn" style="display: flex;align-items: center;justify-content: center;">
            <div class="layui-btn layui-btn-danger layui-btn-md close_window" style="margin-top:10px;" onclick="close_window()"><i class="layui-icon" style="display: none;">&#xe605;</i> 关闭窗口</div>
            <div class="layui-btn layui-btn-success layui-btn-md next_window" style="margin-top:10px;display:none;" onclick="next_piece()">下一篇</div>
            <div class="layui-btn layui-btn-normal layui-btn-md sure_rules" style="margin-top:10px;display:none;" onclick="close_window()">确认协议</div>
        </div>
    </div>
@endif
...
<script>
    ...
    function change_title_color(name){
        ...
        for(let i=0;i<cursor;i++){
            ...
            if(this_text=='《'+name+'》'){
                $('.rule_content').find('.cursor').eq(i).css('color',"#000");
            }
        }
    ...
    function open_check(name,link,method,second,typ){
        ...
        if(typ==1){
            ...
            @foreach($check_content['sure']['title'] as $key2 => $vo)
                if(name=="{{$vo}}") {
                    html += '            <li class="layui-this" lay-method="'+method+'" lay-second="'+second+'" lay-name="{{$vo}}" lay-typ="'+typ+'" lay-id="demo{{$key2}}" lay-nowkey="{{$key2}}" lay-allkey="<?php echo count($check_content['sure']['title']);?>">{{$vo}}</li>\n';
                }else{
                    html+='            <li class="" lay-method="'+method+'" lay-second="'+second+'" lay-name="{{$vo}}" lay-typ="'+typ+'" lay-id="demo{{$key2}}" lay-nowkey="{{$key2}}" lay-allkey="<?php echo count($check_content['sure']['title'])?>">{{$vo}}</li>\n';
                }
            @endforeach
        }
        else if(typ==3){
            @foreach($check_content['knows']['title'] as $key2 => $vo)
                if(name=="{{$vo}}") {
                    html += '            <li class="layui-this" lay-method="'+method+'" lay-second="'+second+'" lay-name="{{$vo}}" lay-typ="'+typ+'" lay-id="demo{{$key2}}" lay-nowkey="{{$key2}}" lay-allkey="<?php echo count($check_content['knows']['title']);?>">{{$vo}}</li>\n';
                }else{
                    html+='            <li class="" lay-method="'+method+'" lay-second="'+second+'" lay-name="{{$vo}}" lay-typ="'+typ+'" lay-id="demo{{$key2}}" lay-nowkey="{{$key2}}" lay-allkey="<?php echo count($check_content['knows']['title'])?>">{{$vo}}</li>\n';
                }
            @endforeach
        }
        ...
        if(typ==1){
            @foreach($check_content['confirm']['link'] as $key2 => $vo)
                if(link=="{{$vo}}") {
                    html += '            <div class="layui-tab-item layui-show">\n';
                }else{
                    html += '            <div class="layui-tab-item @if($key2==0) layui-show @endif">\n';
                }
                html+='        <iframe class="inversion_image" src="{{$vo}}&watch=1" style="width:100%;height:430px;"></iframe><div class="rule_times">\n' +
                ...
                @if(session('user.email')!='')
                    html+='        <span class="f15">确认协议前需把验证码发送到邮箱“<span style="color:#ff2222;font-weight:600;"><?php echo session("user.email");?></span>”，然后把验证码输入下方文本框中。</span>\n' +
                    '        <input type="text" name="number" id="number" value="<?php echo session('user.email');?>" style="display: none;">\n' +
                    '        <input type="text" name="method" id="method" value="1" style="display: none;">\n';
                @elseif(session('user.mobile')!='')
                    html+='        <span class="f15">确认协议前需把验证码发送到手机“<span style="color:#ff2222;font-weight:600;"><?php echo session("user.mobile");?></span>”，然后把验证码输入下方文本框中。</span>\n' +
                    '        <input type="text" name="number" id="number" value="<?php echo session('user.mobile');?>" style="display: none;">\n' +
                ...
                html+='        <div class="input_verifycode">\n' +
                    ...
                    '            <div class="layui-btn layui-btn-danger layui-btn-md close_window" style="margin-top:0px;" onclick="close_window()"><i class="layui-icon" style="display: none;">&#xe605;</i> 关闭窗口</div>\n'+
                    '            <div class="layui-btn layui-btn-normal" id="verifyBtn" onclick="verifyBtn(this,'+typ+')">确认协议</div>\n' +
                    '            <div class="layui-btn layui-btn-normal layui-btn-md next_window" style="margin-top:0px;display:none;" onclick="next_piece()">下一篇</div>\n'+
                    '        </div>\n' +
                    '    </div></div>\n';
            @endforeach
            $('.close_window').show();
            ...
            @foreach($check_content['sure']['link'] as $key2 => $vo)
                if(link=="{{$vo}}") {
                    html += '            <div class="layui-tab-item layui-show">\n';
                }else{
                    html += '            <div class="layui-tab-item @if($key2==0) layui-show @endif">\n';
            ...
            @foreach($check_content['knows']['link'] as $key2 => $vo)
                if(link=="{{$vo}}") {
                    html += '            <div class="layui-tab-item layui-show">\n';
                }else{
                    html += '            <div class="layui-tab-item @if($key2==0) layui-show @endif">\n';
                ...
                html+='        <div class="input_verifycode">\n' +
                    ...
                    '            <div class="layui-btn layui-btn-danger layui-btn-md close_window" style="margin-top:0px;" onclick="close_window()"><i class="layui-icon" style="display: none;">&#xe605;</i> 关闭窗口</div>\n'+
                    '            <div class="layui-btn layui-btn-normal" id="verifyBtn" onclick="verifyBtn(this,'+typ+')">确认协议</div>\n' +
                    '            <div class="layui-btn layui-btn-normal layui-btn-md next_window" onclick="next_piece()" style="display:none;">下一篇</div>\n'+
                    '        </div>\n' +
                    '    </div>\n'+
                    '   </div>\n';
            @endforeach
    ...
    function verifyBtn(t,typ){
        ...
        $.ajax({
            url:"/check_verifyCode_for_rules",
            type:"POST",
            dataType:"json",
            data:{
                "sure_code":sure_code,
