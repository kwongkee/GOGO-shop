<style>
    .category-layer .list .categorys{height:443px !important;overflow-y:auto;overflow-x:hidden;}
    .category-layer .list .categorys .subitems{width: 100%;}
...
@foreach($nav_category as $item)
    <div class="list">
        ...
        <div class="categorys">
                            ...
                            <dd>
                                @if(!empty($v->child))
                                    @foreach($v->child as $child)
                                        <a href="javascript:check_goods('{{ route('pc_goods_list', ['cat_id' => $child->cat_id]) }}',{{$child->cat_id}},'{{ $child->cat_name }}');"  title="{{ $child->cat_name }}">{{ $child->cat_name }}</a>
                                    @endforeach
                                @endif
                            </dd>
                        </dl>
            ...
            <div class="item-right fr" style="width: 0;">
                <!-- 品牌logo -->
    ...
    <script>
        //检查该分类有无商品
        function check_goods(url,cat_id,cat_name){
            $.ajax({
                url: "/check_goods",
                method: 'post',
                data: {'cat_id':cat_id},
                ...
                success: function (res) {
                    if (res.code == 0) {
                        window.location.href="/taozg?cate_name="+cat_name;
                    }else if(res.code==1){
