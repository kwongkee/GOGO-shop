                                    ...
                                    <div class="select_buy_headBox disf">
                                        ...
                                        <dl class="amount amount_num">
                                            ...
                                            <span class="amount-widget">
                                                <input type="text" class="amount-input" value="{{ $sku['start_num'] }}"
                                                       data-sales_model="{{ $goods['sales_model'] }}"
                                                       data-goods_id="{{ $goods['goods_id'] }}"
                                                       data-sku_id="{{ $sku['sku_id'] }}"
                                                       data-amount-min="{{ $sku['start_num'] }}"
...
@endif" onclick="select_services({{$v['id']}},{{$v['type']}},this,'{{$v['name']}}')" @if($v['is_select']==1)
                                        ...
                                        <div class="layui-card-body" style="padding:0;">
                                                ...
                                                <div class="info_div">
                                                    ...
                                                    <div class="layui-form-item">
                                                        ...
                                                        <div class="layui-input-block disf">
                                                            <input type="text" class="layui-input" lay-verify="required" name="user_name1" id="user_name1" value="" placeholder="名">
                                                            <input type="text" class="layui-input" lay-verify="" name="user_name2" id="user_name2" value="" placeholder="中间名">
                                                            <input type="text" class="layui-input" lay-verify="required" name="user_name3" id="user_name3" value="" placeholder="姓">
                                                        </div>
                                                    </div>
                                                    <div class="layui-form-item">
                                                        ...
                                                        <div class="layui-input-block disf">
                                                            ...
                                                            <input type="text" name="mobile" lay-verify="required" placeholder="联系电话" autocomplete="off" class="layui-input" value="">
                                                            <input type="text" name="mobile2" lay-verify="" placeholder="联系电话2" autocomplete="off" class="layui-input" value="">
                                                        </div>
                                                    </div>
                                                    <div class="layui-form-item">
                                                        <div class="layui-form-label">电子邮箱</div>
                                                        <div class="layui-input-block">
                                                            <input type="text" class="layui-input" lay-verify="" name="email" id="email" value="" placeholder="电子邮箱">
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                <input type="hidden" id="address_num" value="1">
                                                <table class="layui-table addr_table">
                                                    <thead>
                                                        <th>详细地址</th>
                                                        <th>操作</th>
                                                    </thead>
                                            ...
                                            <div class="disf" style="justify-content:center;text-align: center;">
                                                ...
                                                <div style="margin-left:10px;">
                                                    <input type="checkbox" name="is_default" id="is_default" lay-skin="primary" title="默认" value="1" checked onclick="is_default(this)" lay-filter="is_default" style="display:none;">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <script type="text/javascript">
                    ...
                    function setSkuInfo(sku) {
                        ...
                        if (goods_number > 0) {
                            if ("1" == 1) {
                                $('.amount_num').find('.amount-input').attr('data-amount-min',sku.sku_prices.start_num[0]);
                                $('.amount_num').find('.amount-input').val(sku.sku_prices.start_num[0]);
                    ...
                    $().ready(function() {
                        ...
                        var goods_number_amount = $(".amount-input").amount({
                            value: $(".amount-input").val(),
                            min: $(".amount-input").attr('data-amount-min'),
                            max: $(".amount-input").attr('data-amount-max'),
                            change: function(element, value) {
                                var sku_id = element.attr('data-sku_id');
                                if (value == this.max) {
                                }
                                var min_value = element.attr('data-amount-min');
                                if(min_value > value){
                                    $.msg("商品数量必须大于" + (min_value - 1));
                                    return false;
                                }
...
{{--                                        <div class="selectBtn">已选</div>--}}
    ...
    <script>
        ...
        function selectAddress(element, addressId) {
            ...
            // 更新显示的地址信息
            var addressItem = element.querySelector('.address-item-content');
            var receiverInfo = addressItem.querySelector('.receiver-info').cloneNode(true);
            var addressDetail = addressItem.querySelector('.address-detail').cloneNode(true);
            
            var addressInfo = document.querySelector('.address-info');
            addressInfo.innerHTML = '';
            
            var selectedDiv = document.createElement('div');
            selectedDiv.className = 'selected-address';
            selectedDiv.appendChild(receiverInfo);
            selectedDiv.appendChild(addressDetail);
            
            addressInfo.appendChild(selectedDiv);
            ...
            addressItems.forEach(function(item) {
                ...
                item.querySelector('.address-select-check').innerHTML = '';
            });
            
            element.classList.add('selected');
            element.querySelector('.address-select-check').innerHTML = '<i class="iconfont">&#xe6b1;</i>';
        }
        
        // 点击外部关闭下拉列表
        document.addEventListener('click', function(event) {
            var addressContainer = document.querySelector('.address-select-container');
            var dropdown = document.getElementById('addressDropdown');
            var arrow = document.querySelector('.address-arrow');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // 如果有默认地址，初始化显示
            var defaultAddress = document.querySelector('.address-item.selected');
            if (defaultAddress) {
                var addressId = defaultAddress.getAttribute('data-address-id');
                var addressSelect = document.getElementById('address_id');
                if (addressSelect) {
