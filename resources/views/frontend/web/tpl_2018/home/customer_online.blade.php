...
<style>
    ...
    .shareBox .link{white-space: pre-wrap;width: 100%;height: 150px;}
                    ...
                    <div class="layui-form-item">
                        <div class="chatContent">
                            ...
                            <div class="disf rightBtm">
                                <div class="chatIcon">
                                    <img src="/assets/d2eace91/images/newhome/chat_upload.png" alt="" id="fileUpload">
                                </div>
                                <div class="chatIcon">
                                    <img src="/assets/d2eace91/images/newhome/chat_share.png?v=23122" alt="" style="width: 26px;height:26px;" id="shareBox" onclick="shareInfo()">
                                </div>
...
<script type="text/javascript">
    layui.use(['layer','form','laydate','upload','element'],function() {
        ...
        upload.render({
            ...
            ,done: function(res){
                ...
                if(res.code == 1)
                {
                    let content = '<a href="https://shop.gogo198.cn/'+ res.file_path +'" target="_blank"><img src="https://shop.gogo198.cn/'+ res.file_path +'" onerror=src="https://shop.gogo198.cn/attachment/images/default_file.png" /></a>';
                    let pid = $('#pid').val();
                    ...
                    $.ajax({
                        url: "/customer_online",
                        method: 'post',
                        data: {'pid':pid,'content_type':2,'pa':1,'who_send':"{{$who_send}}",'_token':"{{csrf_token()}}",'content':content,'origin_page':origin_page},
                        dataType: 'JSON',
                        success: function (res) {
                            layer.msg(res.msg, {time: 2000}, function () {
                                if (res.code == 0) {
    ...
    function sendLink(){
        ...
        $.ajax({
            url: "/customer_online",
            method: 'post',
            data: {'pid':pid,'content_type':2,'pa':1,'who_send':"{{$who_send}}",'_token':"{{csrf_token()}}",'content':content},
            dataType: 'JSON',
            success: function (res) {
                layer.msg(res.msg, {time: 2000}, function () {
                    if (res.code == 0) {
    ...
    function chat(){
        ...
        $.ajax({
            url: "/customer_online",
            method: 'post',
            data: {'pa':2,'who_send':who_send,'_token':"{{csrf_token()}}"},
            dataType: 'JSON',
            success: function (res) {
                            ...
                            if (res.data[i].info[i2]['who_send'] == who_send) {
                                ...
                                if(res.data[i].info[i2]['is_withdraw']==1){
                                    ...
                                    html += '                            <div class="toolbar disf">\n' +
                                        '                                 <div class="quote toolbarImg" onclick="quoteInfo(this,' + res.data[i].info[i2]['id'] + ')"><img src="/assets/d2eace91/images/newhome/chat_quote.png"></div>\n';
                                    if (res.data[i].info[i2]['is_read'] == 0) {
                                        html += '                                 <div class="withdraw toolbarImg" onclick="withdrawInfo(this,' + res.data[i].info[i2]['id'] + ')"><img src="/assets/d2eace91/images/newhome/chat_withdraw.png"></div>';
                                    }
                                    html += '                            </div>\n';
        ...
        if(id>0){
            var confirm_div = layer.confirm('确定要撤回此消息吗？', {
                ...
                $.ajax({
                    url: "/customer_online",
                    method: 'post',
                    data: {'pa': 3, 'id': id, '_token': "{{csrf_token()}}"},
                    dataType: 'JSON',
                    success: function (res) {
                        if(res.code==0){
                            chat();
