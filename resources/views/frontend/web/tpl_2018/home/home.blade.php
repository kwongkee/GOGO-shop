...
<style type="text/css" media="all">
    *{line-height: 24px;}
    ...
    .searchBox .inputBox{height: 100%;width: 100%;box-shadow: 0px 0px 2px 1px {{$website['color_word']}};border-radius: 40px;border:2px solid {{$website['color_word']}};}
    ...
    .navAndContent_feed{align-items: center;display: flex;flex-direction: column;transition: background-color 0.4s ease-in-out, opacity 0.4s ease-in-out;width: 100%;padding-bottom:0px;}
    ...
    #discovery-container{width:100%;height:400px;border-radius:10px;box-shadow: 0px 0px 10px 1px #777;}
...
<style>
    ...
    /*发现轮播 + 切换框====start*/
    .rc_container{max-width: 100% !important;padding:20px 0;}
    ...
    .rc_container .swiper-container {border-radius: 15px;overflow: hidden;box-shadow: 0 10px 30px #777;height: 100%;position: relative;}
    .rc_container .swiper-slide {position: relative;background-size: cover;background-position: center;padding: 0 10px;color: white;}
    .rc_container .slide-overlay {background: rgba(0,0,0,0.5)/*linear-gradient(transparent, rgba(255, 255, 255, 0.8))*/;position: absolute;bottom: 0;left: 0;right: 0;padding: 30px 20px 20px;}
    ...
    .rc_container .news-content {flex-grow: 1;overflow: hidden;position: relative;height: 350px;}
    ...
    .rc_container .customer-service {margin-top: 0px;background: white;border-radius: 15px;overflow: hidden;box-shadow: 0 5px 15px #777;display:none;height:350px;}
    ...
    .rc_container .service-content {padding: 10px;text-align: center;color: #888;font-style: italic;height: 100%;display: flex;align-items: center;justify-content: center;}
    ...
    @media (max-width: 768px) {
        ...
        .rc_container .swiper-container {height: 100%;}
    ...
    /*原新闻区域（废弃）start*/
    /*热卖*/
    .rc_container .hotbuy_div .goodsDiv .goodsImg{width:calc(20% - 10px);height:50px;margin-right:10px;border:2px solid #777;}
    .rc_container .hotbuy_div .goodsDiv .goodsInfo{width:80%;}
    .rc_container .hotbuy_div .goodsDiv .goodsInfo .disf{justify-content: space-between;padding: 0 20px 0 0;}
    /* 滚动容器：固定宽度，超出隐藏，作为滚动视口 */
    .rc_container .hotbuy_div .goodsDiv .goodsInfo .scroll-container {overflow: hidden;white-space: nowrap;width: 100%;box-sizing: border-box;}
    /* 滚动文本：动画从左向右滚动（实际上是向左移动） */
    .rc_container .hotbuy_div .goodsDiv .goodsInfo .scroll-text {display: inline-block;white-space: nowrap;animation: scroll-left 15s linear infinite;}
    /* 关键帧：从初始位置 0 滚动到自身宽度的负值（完全移出左侧） */
    @keyframes scroll-left {
        0% {transform: translateX(0);}
        100% {transform: translateX(-100%);}
    }
    .rc_container .hotbuy_div .goodsDiv .goodsInfo .title{width: 100%;white-space: nowrap;color:#000;font-weight:800;}
    .rc_container .hotbuy_div .goodsDiv .goodsInfo .price{;color:{{$website['color']}};padding: 3px 10px;border-radius: 5px;width: 110px;text-align: center;margin-top:5px;border:1px solid {{$website['color_word']}};white-space:nowrap;font-weight:800;}
    .rc_container .hotbuy_div .viewGoods{background:{{$website['color']}};color:{{$website['color_word']}};padding: 3px 10px;border-radius: 5px;width: fit-content;text-align: center;margin-left:15px;cursor:pointer;margin-top:5px;border-radius:20px;font-weight:800;}
    /*发现轮播 + 切换框====end*/
    ...
    .cross-border .section-title {text-align: left;margin-bottom: 0px;}
    ...
    /* 头部切换区域 */
    .change_boxClick .tab-header {display: flex;align-items: center;justify-content: center;position: relative;width:95%;max-width: 100%;margin: 0 auto;padding:0px;border-radius:0px;border: 0px solid #fff;margin-top:10px;}
    ...
    /* 左右箭头 */
    .change_boxClick .arrow {position: absolute;top: 50%;transform: translateY(-50%);width: 30px;height: 30px;background-color: rgba(255, 255, 255, 0.8);border: 1px solid #e9ecef;border-radius: 4px;cursor: pointer;display: none;justify-content: center;align-items: center;font-size: 16px;color: #495057;}
    ...
    /* 常见问题项容器样式2====start*/
    .fq_container .categories {display: flex;border-bottom: 2px solid #e0e0e0;}
    ...
    .card3_style .containers {display: grid;grid-template-columns: repeat(2, 1fr);justify-content: space-around;gap: 30px;}
    /* 卡片通用样式 */
    ...
    .card3_style .card-icon img{width:100%;height:100%;border-radius: 15px;filter: drop-shadow(0 8px 12px rgba(0, 0, 0, 0.2));transition: transform 0.4s ease, filter 0.3s ease;}
    ...
    .storeDiv{width: 100%;margin: 0px 0px 0px;padding: 0px;position: relative;box-sizing: border-box;border-radius: 5px;border-top-left-radius: 0;}
    ...
    .storeDiv .storeSwiper{padding:0 !important;}
    .storeDiv .column4_list{width:100%;}
    .storeDiv .hsBox{border: 3px solid #777;width: 100%;box-shadow: 0px 0px 8px 0px rgba(0,0,0,0.4);border-top-left-radius: 10px;border-top-right-radius: 10px;}
    .storeDiv .hsBox .hsDiv{width:100%;height: 255px;position: relative;overflow:hidden;border-top-left-radius: 5px;border-top-right-radius: 5px;}
    ...
    .storeDiv .hsBox .hsDiv .hsMask{width: 100%;height: 25%;position: absolute;top: 0;background: #444;opacity: 0.4;z-index: 8;border-radius: 0px;border-top-left-radius: 0;border-top-right-radius: 0;display:none;}
    ...
    .storeDiv .hsBox .hsDiv .hsContent .title{font-size:17px;font-weight: 800;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 1;overflow: hidden;text-overflow: ellipsis;margin-bottom:10px;min-height:28px;max-height:28px;display:none;}
    .storeDiv .hsBox .hsDiv .hsContent hr{ border-bottom: 1px solid {{$website['color_word']}};width: 75px;margin: 0 0 10px;display:none;}
    ...
    .storeDiv .about-desc {font-size: 14px;margin-top: 0px;padding: 10px 20px;box-sizing: border-box;background: {{$website['color']}};color: {{$website['color_word']}};opacity: 1;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;min-height:132px;border-bottom-left-radius: 10px;border-bottom-right-radius: 10px;}
    .storeDiv .about-desc .about-desc-div{line-height: 1.5;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;min-height:48px;font-size:15px;font-weight:800;}
    .storeDiv .moreBtn{justify-content:space-between;margin-top:25px;}
    .storeDiv .moreBtn .priceDiv{font-size: 15px;font-weight: 600;}
    .storeDiv .view_detail {display: inline-block;padding: 5px 15px;background: {{$website['color']}};backdrop-filter: blur(2px);border: 2px solid {{$website['color_word']}};border-radius: 40px;color: {{$website['color_word']}};font-weight: 600;text-decoration: none;transition: all 0.3s ease;margin-top: 0px;letter-spacing: 0.5px;position:unset;transform:unset;}
    .storeDiv .view_detail:hover {background: {{$website['color']}};border-color: {{$website['color_word']}};color:{{$website['color_word']}};}

    @media (max-width: 992px){
        .store_body .container{max-width:400px;}
        .storeDiv{margin:20px 0px 0px;padding:0;}
        .storeDiv .storeSwiper{padding:0 !important;}
        .storeDiv .hsBox .hsDiv{height: 250px;}
        ...
        .storeDiv .hsBox .hsDiv .hsContent .title{-webkit-line-clamp:1;margin-bottom: 6px;min-height: 25px;max-height: 25px;font-size:16px;}
    }
    /*标题+图片（一排四个）卡片样式====end*/

    /*标题+图片（一排三个）卡片样式====start*/
    .cardDiv {width: 100%;margin: 0px 0px 0px;position: relative;box-sizing: border-box;border-radius: 5px;border-top-left-radius: 0;}
    ...
    .cardDiv .cross-section{background-position: center;background-size: cover;position: relative;transform-style: preserve-3d;box-shadow: 0px 0px 10px rgba(0,0,0,0.4);overflow:hidden;height:100% !important;border-top-left-radius: 10px;border-top-right-radius: 10px;}
    .cardDiv .cross-section .in_mask {background-color: #444;opacity: 0.4;position: absolute;left: 0;top: 0;height: 25%;width: 100%;z-index: 1;display:none;}
    .cardDiv .cross-section .cross-content hr {border-bottom: 1px solid {{$website['color_word']}};width: 75px;margin: 0 0 10px;display:none;}
    .cardDiv .cross-section .cross-content {padding: 20px 30px 80px 30px;z-index: 2;position: relative;font-size: 15px;height: 100%;min-height: 375px;max-height: 375px;border:3px solid #777;border-top-left-radius:9px;border-top-right-radius:9px;}
    .cardDiv .cross-section .cross-content h3 {color:{{$website['color_word']}};margin-bottom: 10px;padding-bottom: 0;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;font-weight:1000;display:none;}
    ...
    .cardDiv .about-desc{font-size:18px;margin-top:0px;background:;padding:10px 20px;box-sizing: border-box;background: {{$website['color']}};color: {{$website['color_word']}};border-bottom-left-radius: 10px;border-bottom-right-radius: 10px;}
    .cardDiv .moreBtn {justify-content: space-between;margin-top: 25px;}
    .cardDiv .moreBtn .priceDiv {font-size: 15px;font-weight: 600;}
    .cardDiv .about-desc .about-desc-div{line-height: 1.5;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;min-height: 48px;font-size: 15px;font-weight: 800;}
    .cardDiv .view_detail {display: inline-block;padding: 5px 15px;background: {{$website['color']}};backdrop-filter: blur(2px);border: 2px solid {{$website['color_word']}};border-radius: 40px;color: {{$website['color_word']}};font-weight: 600;text-decoration: none;transition: all 0.3s ease;margin-top: 0px;font-size: 15px;letter-spacing: 0.5px;position:unset;transform:unset;}
    ...
    .festivalDiv {width: 100%;position: relative;box-sizing: border-box;border-radius: 5px;border-top-left-radius: 0;}
    ...
    .festivalDiv .hs2 {border: 3px solid #777;width: 100%;border-top-left-radius: 10px;border-top-right-radius: 10px;}
    .festivalDiv .hsDiv .guide2_country_img {width: 100%;max-width: 100%;height: -webkit-fill-available;}
    .festivalDiv .hsMask {width: 100%;height: 35%;position: absolute;top: 0;background: #444;opacity: 0.4;z-index: 8;border-radius: 0px;}
    ...
    .festivalDiv .hsDiv .hsContent .moreBtn {width: 100%;text-align: center;}
    .festivalDiv .about-desc{font-size:15px;margin-top:0px;padding:10px 20px;box-sizing: border-box;background: {{$website['color']}};color: {{$website['color_word']}};border-bottom-left-radius: 10px;
  border-bottom-right-radius: 10px;}
    .festivalDiv .about-desc .about-desc-div{line-height: 1.5;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;min-height:50px;font-weight:800;}
    .festivalDiv .moreBtn{text-align:right;}
    .festivalDiv .view_detail {display: inline-block;padding: 5px 15px;background: {{$website['color']}};border: 2px solid {{$website['color_word']}};border-radius: 40px;color: {{$website['color_word']}};font-weight: 500;text-decoration: none;transition: all 0.3s ease;margin-top: 10px;font-size: 15px;letter-spacing: 0.5px;position:unset;transform:unset;}
    ...
    /*杂志导航样式====start*/
    .industryDiv {width: 100%;position: relative;box-sizing: border-box;border-radius: 5px;border-top-left-radius: 0;}
    ...
    .industryDiv .serviceBox .leftBox .cont6 .serviceContent .serviceDiv .serviceIn{font-weight:800;margin-top:15px;width: fit-content;color: {{$website['color_word']}};font-size: 15px;background: {{$website['color']}};padding: 4px 15px;border-radius: 15px;border: 2px solid {{$website['color_word']}};transition: all var(--transition-fast);}
    .industryDiv .serviceBox .leftBox .cont6 .serviceContent .serviceDiv .serviceIn:hover{transform: translateY(-8px);box-shadow: var(--shadow-hover);}
    ...
    .industryDiv .serviceBox .rightBox .serviceContent .serviceDiv{width:-webkit-fill-available;height:274px;border:2px solid {{$website['color_word']}};border-radius: 8px;box-shadow: 0px 0px 8px 0px rgba(0,0,0,0.4);padding:22px 22px 22px 32px;background: #a49f9f;position:relative;}
    ...
    .industryDiv .serviceBox .rightBox .serviceContent .serviceDiv .serviceTxt .moreBtn{width: 100%;text-align: right;position:absolute;bottom: 10px;right: 5px;transition: all var(--transition-fast);}
    .industryDiv .serviceBox .rightBox .serviceContent .serviceDiv .serviceTxt .moreBtn a{color:{{$website['color_word']}};font-size: 15px;background: {{$website['color']}};padding: 4px 15px;border-radius: 15px;border: 2px solid {{$website['color_word']}};}
    .industryDiv .serviceBox .rightBox .serviceContent .serviceDiv .serviceTxt .moreBtn:hover{transform: translateY(-8px);/*box-shadow: var(--shadow-hover);*/}
    ...
    @media (max-width: 992px){
        .industryDiv{margin-top:20px;padding:0;}
        .industry_body .container{width:400px;}
        .industryDiv .serviceBox{display:block;}
        .industryDiv .serviceBox .rightBox{display:none;}
        .industryDiv .serviceBox .leftBox{width:100%;height:410px;min-height:320px;margin-bottom:0;}
        ...
        .industryDiv .serviceBox .leftBox .cont6 .serviceContent .serviceDiv .mob_cont6 .c6Title a{color:{{$website['color_word']}};}
    }
    /*杂志导航样式====end*/
...
:root {
    /* 现代渐变配色 */
    --primary-gradient: {{$website['color']}};
    --secondary-gradient: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);
    --accent-gradient: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);
    
    /* 柔和阴影体系 */
    --shadow-sm: 0 2px 8px rgba(0, 0, 0, 0.08);
    --shadow-md: 0px 0px 10px 1px #777;
    --shadow-lg: 0 16px 48px rgba(0, 0, 0, 0.16);
    --shadow-hover: 0 20px 60px rgba(0, 0, 0, 0.2);
    
    /* 玻璃拟态效果 */
    --glass-bg: {{$website['color']}};
...
.searchBox .inputBox .nameBox input {
    font-size: 16px;
    color: #333;
    padding: 12px 20px;
}

.searchBox .inputBox .btnBox {
    background: var(--primary-gradient);
    border-radius: 0 var(--radius-xl) var(--radius-xl) 0;
    transition: all var(--transition-fast);
}

.searchBox .inputBox .btnBox:hover {
...
.column_item .content_card:hover {
    transform: translateY(-8px);
    box-shadow: var(--shadow-hover);
}

.column_item .content_card img {
    border-radius: var(--radius-lg) var(--radius-lg) 0 0;
    transition: transform var(--transition-slow);
}

.column_item .content_card:hover img {
    transform: scale(1.08);
}

.column_item .content_card .content_body {
...
/* ============ 板块标题美化 ============ */
.cross-border .section-title a h2 {
    background: var(--primary-gradient);
...
.cross-border .section-title a h2::after {
    background: var(--primary-gradient);
    height: 4px;
    border-radius: 2px;
}
...
.rc_container .news-slide {
    padding: 16px 20px;
    border-bottom: 1px solid #f0f0f0;
    transition: all var(--transition-fast);
}

.rc_container .news-slide:hover {
    background: linear-gradient(90deg, #f8f9ff 0%, #fff 100%);
    transform: translateX(8px);
    border-left: 3px solid #667eea;
}

.rc_container .news-title::before {
    content: '●';
    color: #667eea;
    font-size: 8px;
    margin-right: 12px;
...
.column_item:nth-child(1) { animation-delay: 0.1s; }
.column_item:nth-child(2) { animation-delay: 0.2s; }
.column_item:nth-child(3) { animation-delay: 0.3s; }
.column_item:nth-child(4) { animation-delay: 0.4s; }
.column_item:nth-child(5) { animation-delay: 0.5s; }
.column_item:nth-child(6) { animation-delay: 0.6s; }
    ...
    <section class="navAndContent navAndContent_tb navAndContent_nb">
        <div class="navAndContent_feed">
            ...
            @foreach($services as $k=>$vo)
                @if($vo['navbar_id']=='A1' || $vo['format_type']==3)
                    <!--小轮播和切换框-->
                            ...
                            <div class="content">
                                    ...
                                    <div class="swiper-container" id="discovery-container">
                                        <div class="swiper-wrapper">
                                            @foreach($discovery_rotate as $k=>$vo)
                                                <div class="swiper-slide" style="background-image: url('https://shop.gogo198.cn/{{$vo['thumb']}}');" onclick="javascript:window.location.href='@if($vo['go_other']==1) {{$vo['other_link']}} @elseif($vo['go_other']==3) /txt_detail?id={{$vo['other_pic']}}&type=1&oid={{$vo['id']}} @elseif($vo['go_other']==4) /msg_detail?id={{$vo['other_msg']}}&type=1&oid={{$vo['id']}} @elseif($vo['go_other']==5) /goods_list?frame_id=4&hotsearchId={{$vo['id']}}&searchTitle={{$vo['other_keywords']}} @endif';">
                                        ...
                                        <!--左右切换-->
                                        <div class="swiper-button-next sn_discovery"></div>
                                        <div class="swiper-button-prev sp_discovery"></div>
                                ...
                                <div class="right-section">
                                    <div class="tab-container">
                                        <div class="tab active f18" data-tab="news">全网热卖</div>
                                        <div class="tab f18" data-tab="service">客服中心</div>
                                    </div>
                                    <div class="news-container">
                                        <div class="news-content">
                                            <div class="swiper-container hotbuy_div">
                                                <div class="swiper-wrapper">
                                                    @foreach($hotbuy as $k=>$vo2)
                                                        @if(!empty($vo2['mainItemImgs']))
                                                            <div class="swiper-slide">
                                                                <div class="goodsDiv">
                                                                    <a href="/goods-{{$vo2['goods_id']}}.html" target="_blank">
                                                                        <div class="disf">
                                                                            <div class="goodsImg" style="background: url({{$vo2['mainItemImgs'][0]['path']}}) no-repeat 100% 100%;background-size: cover;"></div>
                                                                            <div class="goodsInfo">
                                                                                <!--<div class="title f15" title="{{$vo2['goods_name']}}">{{$vo2['goods_name']}}</div>-->
                                                                                <div class="scroll-container">
                                                                                    <div class="scroll-text title f15" title="{{$vo2['goods_name']}}">
                                                                                        {{$vo2['goods_name']}}
                                                                                    </div>
                                                                                </div>
                                                                                <div class="disf">
                                                                                    <div class="price f15">{{$vo2['currency']}} {{$vo2['goods_price']}}</div>
                        ...
                        <section class="cross-border store_body ass" style="background:@if($vo['bg_type']==1) {{$vo['bg_color']}} @elseif($vo['bg_type']==2) url(https://shop.gogo198.cn/{{$vo['bg_img']}}) @endif;">
                            <div class="container">
                                <div class="row" style="margin-bottom:0;">
                                    <div class="col-md-12">
                                        <div class="section-title text-center">
                                            <a href="/goods_list?hotsearchId={{$vo['info']['id']}}&frame_id=5" target="_blank" class="f15"><h2 class="f26">{{$vo['info']['name']}}</h2></a>
                                            <p style="margin-bottom:10px;" class="f15">{{$vo['info']['desc']}}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="storeDiv">
                                    <div class="swiper-container{{$vo['id']}} storeSwiper factorySwiper{{$vo['id']}}" style="overflow-y:visible;box-sizing: border-box;overflow:hidden;">
                                        <div class="swiper-wrapper">
                                            @foreach($vo['children'] as $k2=>$v2)
                                                <div class="swiper-slide">
                                                    ...
                                                    <div class="about-desc">
                                                        <div class="about-desc-div">{{$v2['desc']}}</div>
                                                        <div class="moreBtn disf">
                                                            @if($v2['go_other']==1 || $v2['go_other']==6 || $v2['go_other']==7)
                                                                <!--分享&我要咨询&去找客服-->
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="javascript:common_operation({{$v2['go_other']}},this);" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==2)
                                                                ...
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="//www.gogo198.cn/goods-{{$v2['other_goods']}}.html" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==3)
                                                                <!--菜单链接-->
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="/detail?id={{$v2['other_navbar']}}" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==4)
                                                                <!--第三方链接-->
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="{{$v2['link']}}" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==5)
                                                                <!--搜索关键字-->
                                                                <?php $frame_id=1;?>
                                                                @if($vo['bind_festival']==1)
                                                                    <?php $frame_id=2;?>
                                                                @endif
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="/goods_list?frame_id={{$frame_id}}&hotsearchId={{$v2['id']}}&searchTitle={{$v2['other_keywords']}}" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==8)
                                                                <!--图文-->
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="/txt_detail?id={{$vo2['other_pic']}}&type=image_txt&oid={{$vo2['other_pic']}}" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @elseif($v2['go_other']==9)
                                                                <!--消息-->
                                                                <div class="detailDiv">
                                                                    <a class="view_detail f15" href="/msg_detail?id={{$vo2['other_msg']}}&type=image_txt&oid={{$vo2['other_msg']}}" target="_blank">详情&gt;</a>
                                                                </div>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="cross_border_content">
                                                        ...
                                                        <div class="about-desc">
                                                            ...
                                                            @if(empty($v2['children']))
                                                                @if($v2['go_other']==1 || $v2['go_other']==6 || $v2['go_other']==7)
                                                                    <!--分享&我要咨询&去找客服-->
                                                                    <a href="javascript:common_operation({{$v2['go_other']}},this);" class="f15 view_detail">详情&gt;</a>
                                                                @elseif($v2['go_other']==2)
                                                                    <!--商品链接-->
                                                                    <div class="moreBtn disf">
                                                                        <div class="priceDiv disf">
                                                                        ...
                                                                        <a href="//www.gogo198.cn/goods-{{$v2['other_goods']}}.html" target="_blank" class="f15 view_detail">详情&gt;</a>
                                                                    </div>
                                                                @elseif($v2['go_other']==3)
                                                                    <!--菜单链接-->
                                                                    <a href="/detail?id={{$v2['other_navbar']}}" target="_blank" class="f15 view_detail">详情&gt;</a>
                                                                @elseif($v2['go_other']==4)
                                                                    <!--第三方链接-->
                                                                    <a href="{{$v2['link']}}" target="_blank" class="f15 view_detail">详情&gt;</a>
                                                                @elseif($v2['go_other']==5)
                                                                    <!--搜索关键字-->
                                                                    <?php $frame_id=1;?>
                                                                    @if($vo['bind_festival']==1)
                                                                        <?php $frame_id=2;?>
                                                                    @endif
                                                                    <a href="/goods_list?frame_id={{$frame_id}}&hotsearchId={{$v2['id']}}&searchTitle={{$v2['gkeywords']}}" target="_blank" class="f15 view_detail">详情&gt;</a>
                                                                @elseif($v2['go_other']==8)
                                                                    <!--图文-->
                                                                    <a href="/txt_detail?id={{$vo2['other_pic']}}&type=image_txt&oid={{$vo2['other_pic']}}" target="_blank">详情&gt;</a>
                                                                @elseif($v2['go_other']==9)
                                                                    <!--消息-->
                                                                    <a href="/msg_detail?id={{$vo2['other_msg']}}&type=image_txt&oid={{$vo2['other_msg']}}" target="_blank">详情&gt;</a>
                                                                @endif
                                                            @else
                                                                <ul class="withArrow">
                                                        ...
                                                        <div class="hsColumn">
                                                            @foreach($v3 as $k4=>$v4)
                                                                <div class="hsBox">
                                                                    ...
                                                                    <div class="about-desc">
                                                                        <div class="about-desc-div">{{$v4['desc']}}</div>
                                                                        <div class="moreBtn">
                                                                            @if($v4['go_other']==1 || $v4['go_other']==6 || $v4['go_other']==7)
                                                                                <!--分享&我要咨询&去找客服-->
                                                                                <a class="view_detail" href="javascript:common_operation({{$v4['go_other']}},this);">详情&gt;</a>
                                                                            @elseif($v4['go_other']==2)
                                                                                <!--商品链接-->
                                                                                <a class="view_detail" href="//www.gogo198.cn/goods-{{$v4['other_goods']}}.html" target="_blank">详情&gt;</a>
                                                                            @elseif($v4['go_other']==3)
                                                                                <!--菜单链接-->
                                                                                <a class="view_detail" href="/detail?id={{$v4['other_navbar']}}" target="_blank">详情&gt;</a>
                                                                            @elseif($v4['go_other']==4)
                                                                                <!--第三方链接-->
                                                                                <a class="view_detail" href="{{$v4['link']}}" target="_blank">详情&gt;</a>
                                                                            @elseif($v4['go_other']==5)
                                                                                ...
                                                                                @if($vo['bind_festival']==1)
                                                                                    <?php $frame_id=2;?>
                                                                                @endif
                                                                                <a class="view_detail" href="/goods_list?frame_id={{$frame_id}}&hotsearchId={{$v4['id']}}&searchTitle={{$v4['gkeywords']}}" target="_blank">详情&gt;</a>
                                                                            @elseif($v4['go_other']==8)
                                                                                <!--图文-->
                                                                                <a href="/txt_detail?id={{$v4['other_pic']}}&type=image_txt&oid={{$v4['other_pic']}}" target="_blank" class="view_detail">详情&gt;</a>
                                                                            @elseif($v4['go_other']==9)
                                                                                <!--消息-->
                                                                                <a href="/msg_detail?id={{$v4['other_msg']}}&type=image_txt&oid={{$v4['other_msg']}}" target="_blank" class="view_detail">详情&gt;</a>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <div class="serviceDiv">
                                                                <div class="disf">
                                                                    ...
                                                                    <div class="serviceTxt">
                                                                        ...
                                                                        @if($v4['go_other']==1 || $v4['go_other']==6 || $v4['go_other']==7)
                                                                            <div class="moreBtn"><a href="javascript:common_operation({{$v4['go_other']}},this);">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==2)
                                                                            <!--商品链接-->
                                                                            <div class="moreBtn"><a href="///www.gogo198.cn/goods-{{$v4['other_goods']}}.html" target="_blank">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==3)
                                                                            <!--菜单链接-->
                                                                            <div class="moreBtn"><a href="/detail?id={{$v4['other_navbar']}}" target="_blank">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==4)
                                                                            <!--第三方链接-->
                                                                            <div class="moreBtn"><a href="{{$v4['link']}}" target="_blank">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==5)
                                                                            <!--搜索关键字-->
                                                                            <div class="moreBtn"><a href="/goods_list?frame_id=1&hotsearchId={{$v4['id']}}&searchTitle={{$v4['gkeywords']}}" target="_blank">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==8)
                                                                            <!--图文-->
                                                                            <div class="moreBtn"><a href="/txt_detail?id={{$v4.other_pic}}&type=image_txt&oid={{$v4.other_pic}}" target="_blank">详情&gt;</a></div>
                                                                        @elseif($v4['go_other']==9)
                                                                            <!--消息-->
                                                                            <div class="moreBtn"><a href="/msg_detail?id={{$v4.other_msg}}&type=image_txt&oid={{$v4.other_msg}}" target="_blank">详情&gt;</a></div>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                            </div>
                    ...
                    @elseif($vo['format_type']==13)
                        ...
                        <section class="cross-border store_body ass" style="background:@if($vo['bg_type']==1) {{$vo['bg_color']}} @elseif($vo['bg_type']==2) url(https://shop.gogo198.cn/{{$vo['bg_img']}}) @endif;">
                            <div class="container">
                                ...
                                <div class="cardDiv">
                                    <div class="swiper-container{{$vo['id']}} cardSwiper{{$vo['id']}} swiper-container-horizontal" style="overflow:hidden;position:relative;box-sizing:border-box;overflow:hidden;">
                                        <div class="swiper-wrapper">
                                            @foreach($vo['children'] as $k2=>$v2)
                                            <div class="swiper-slide" style="height:100%;">
                                                <div class=" cross_border_content">
                                                    ...
                                                    <div class="about-desc">
                                                        ...
                                                        <div class="moreBtn disf">
                                                            @if($v2['go_other']==1 || $v2['go_other']==6 || $v2['go_other']==7)
                                                                <!--分享&我要咨询&去找客服-->
                                                                <a class="view_detail" href="javascript:common_operation({{$v2['go_other']}},this);">详情&gt;</a>
                                                            @elseif($v2['go_other']==2)
                                                                <!--商品链接-->
                                                                <div class="priceDiv disf">
                                                                    <div class="currency">{{$v2['info']['currency']}}</div>
                                                                ...
                                                                <a class="view_detail" href="//www.gogo198.cn/goods-{{$v2['other_goods']}}.html" target="_blank">详情&gt;</a>
                                                            @elseif($v2['go_other']==3)
                                                                <!--菜单链接-->
                                                                <a class="view_detail" href="/detail?id={{$v2['other_navbar']}}" target="_blank">详情&gt;</a>
                                                            @elseif($v2['go_other']==4)
                                                                <!--第三方链接-->
                                                                <a class="view_detail" href="{{$v2['link']}}" target="_blank">详情&gt;</a>
                                                            @elseif($v2['go_other']==5)
                                                                <!--搜索关键字-->
                                                                <?php $frame_id=1;?>
                                                                @if($vo['bind_festival']==1)
                                                                    <?php $frame_id=2;?>
                                                                @endif
                                                                <a class="view_detail" href="/goods_list?frame_id={{$frame_id}}&hotsearchId={{$v2['id']}}&searchTitle={{$v2['gkeywords']}}" target="_blank">详情&gt;</a>
                                                            @elseif($v2['go_other']==8)
                                                                <!--图文-->
                                                                <a href="/txt_detail?id={{$v2['other_pic']}}&type=image_txt&oid={{$v2['other_pic']}}" target="_blank" class="view_detail">详情&gt;</a>
                                                            @elseif($v2['go_other']==9)
                                                                <!--消息-->
                                                                <a href="/msg_detail?id={{$v2['other_msg']}}&type=image_txt&oid={{$v2['other_msg']}}" target="_blank" class="view_detail">详情&gt;</a>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(function(){
        ...
        if(!IsPhone()){
            $('.cross-border').css({'height': viewportHeight - 100 + 'px'});// 业务服务
            $('.festival_body').css({'height': 'auto','padding-bottom': '70px'});
            $('.store_body,.industry_body').css({'height':'auto'});
            
            var viewportHeight2 = viewportHeight - 350;
            $('.cross-border .row .cross-section .cross-content').css({'height': viewportHeight2 + 110 + 'px'});
        }else{
            $('.cross-border').css({'height': plateHeight - 200 + 'px'});// 业务服务
            $('.festival_body').css({'height': 'auto','padding-bottom': '40px'});
