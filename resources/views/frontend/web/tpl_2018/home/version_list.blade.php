...
@section('content')
    ...
    <style type="text/css" media="all">
        #content{padding-top:120px;padding-bottom:20px;background:{{$website['background']}};}
                    ...
                    <div class="layui-fluid" style="padding:0;">
                        <div class="layui-row layui-col-space15">
                            <div class="layui-col-md12" style="padding:0;">
                                <p class="navbar_menu"><i class="fa fa-sign-in" style="margin-right:5px;display:none;"></i><a href="/">HOME</a>&nbsp;<span style="color:{{$website['fontcolor']}};">\</span>&nbsp;<a href="/rule_list" style="color:{{$website['fontcolor']}};">平台规则</a>&nbsp;<span style="color:{{$website['fontcolor']}};">\</span>&nbsp;<span style="color:{{$website['fontcolor']}};">{{$history['rule_name']}}</span>&nbsp;</p>
    ...
    <script>
        layui.use(['layer', 'form', 'table', 'upload'], function () {
            ...
            table.render({
                elem: '#mainTable'
                ,url: "/version_list?pa=1&pid={{$pid}}"
                ,cellMinWidth: 200
                ,cols: [[
                    {field:'createtime', title: '发布日期'}
                    ,{field:'effecttime', title: '生效日期'}
                    ,{field:'version', title: '版本编号'}
                    ,{align:'center',  title: '规则',fixed:'right',width:120, templet: function(d){
                            return [
                                '<a onclick="openWindow('+"'"+ d.id +"'" +','+"1"+ ');" class="layui-btn layui-btn-primary layui-btn-xs" style="color:#0e2e68;border:1px solid #0e2e68;">查看规则</a>',
        ...
        function openWindow(id,typ,type_id){
            let url = '';
            if(typ==1){
                window.location.href="/rule_detail?pid={{$pid}}&id="+id;
            }
        }
    </script>
@stop
