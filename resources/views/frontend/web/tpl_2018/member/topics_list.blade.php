...
<style type="text/css">
    ...
    @media screen and (max-width: 450px){
        .share_topics_btn{margin-top:10px !important;margin-left:0 !important;}
    ...
    .layui-colla-title2 .num{white-space:nowrap;width:20%;text-align: center;margin-right: 3%;border-right: 1px solid {{$website['color_word']}};}
        ...
        <div class="layui-card-body" style="padding: 10px  10px;">
            <form class="layui-form" action="" lay-filter="component-form-group">
                ...
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 30px 0;">
                    ...
                    @foreach($topics_list as $key=>$vo)
                    <div class="layui-colla-item">
                        <h2 class="layui-colla-title2 disf">
                            <div class="num">议题{{$key}}.</div>
                            <div class="title">{{$vo['name']}}</div>
                            <div class="arrow_div @if($vo['is_quanyuan']==0)
                                    @if($vo['is_ending']==0)
                                    r_blue
                                    @elseif($vo['is_ending']==1)
                                    r_grey
                                    @endif
                                    @elseif($vo['is_quanyuan']==1)
                                    @if($vo['is_pass']==0)
                                    @if($vo['is_ending']==0)
                                    r_red1
                                    @elseif($vo['is_ending']==1)
                                    ...
                                    @endif
                                    @endif
                                    @endif" onclick="change_colla(this)">
                                <div class="@if($key==1)
                                        arrow2
                                    @else
                                        arrow
                        ...
                        <div class="layui-colla-content @if($key==1)
                                layui-show
                            @endif">
                            <div class="txt_btn" onclick="taolun({{$vo['id']}},'{{$vo['name']}}',{{$vo['is_ending']}})">讨论</div>
                            <div class="txt_btn" onclick="resolution({{$vo['id']}},'{{$vo['name']}}',{{$vo['is_ending']}})">决议</div>
                            @if($vo['user_id']==session('account.id'))
                            <div class="txt_btn" onclick="edit_topics({{$vo['is_ending']}},'{{$vo['name']}}',{{$vo['already_resolution']}},{{$vo['id']}})">更改</div>
                            <div class="txt_btn" onclick="share_topics({{$vo['is_ending']}},'{{$vo['name']}}',{{$vo['id']}})">分享</div>
                            @endif
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 30px 0;">
                    ...
                    <div class="layui-colla-item">
                        ...
                        <div class="layui-colla-content @if($key==1)
                                layui-show
                            @endif">
                            <div class="txt_btn" onclick="resolution({{$vo['id']}},'{{$vo['name']}}',{{$vo['is_ending']}})">查看</div>
                            @if($vo['user_id']==session('account.id'))
                            <div class="txt_btn" onclick="edit_topics({{$vo['is_ending']}},'{{$vo['name']}}',{{$vo['already_resolution']}},{{$vo['id']}})">更改</div>
                            <div class="txt_btn" onclick="share_topics({{$vo['is_ending']}},'{{$vo['name']}}',{{$vo['id']}})">分享</div>
                            @endif
...
<script>
    layui.config({
        ...
        form.on('select(select_enterprise)',function(data){
            let val = data.value;
            if(val != ''){
                $.post('/member/join_group',{'group_id':"{{$is_join['group_id']}}",'status':val},function(res){
    ...
    function taolun(id,name,is_ending){
        if(is_ending==0){
            //正在进行
            var layer = layui.layer;
            var index = layer.open({
                type: 2,
                title: name,
                content: '/customer/discuss_online&id='+id,
                area:['100%','100%']
            });
        }
    }

    function resolution(id,name,is_ending){
        if(is_ending==0) {
            //正在进行
            var layer = layui.layer;
            var index = layer.open({
                type: 2,
                title: name,
                content: '/member/topics_detail&id=' + id + '&is_edit=<?php echo base64_encode($is_edit);?>',
                area: ['100%', '100%']
            });
        }
    }

    function share_topics(is_ending,name,id){
        var layer = layui.layer,$ = layui.$;
        layer.load();
        $.post('/member/share_topics',{'id':id},function(res){
            layer.closeAll('loading');
            if(res.code==0){
                var index = layer.open({
                    type: 1,
                    title: name,
                    content: '<div style="padding:10px;box-sizing: border-box;text-align: center;"><img src="'+res.img+'" style="width: 60%;"><p style="font-size: 16px;margin-top:10px;">长按图片保存</p></div>',
                    area:['250px','250px']
                });
            }
        });
    ...
    function edit_topics(is_ending,name,already_resolution,id){
        var layer = layui.layer;
        if(already_resolution==0){
            //未发生投票
            var index = layer.open({
                type: 2,
                title: name,
                content: '/merchant/save_topics&id='+id+'&method=3',
                area:['100%','100%']
            });
        }
        else if(already_resolution==1){
            //已发生投票
            var index = layer.open({
                type: 2,
                title: name,
                content: '/merchant/save_topics&id='+id+'&method=4',
                area:['100%','100%']
            });
        }
    }
