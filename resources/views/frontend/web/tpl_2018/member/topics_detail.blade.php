        ...
        <div class="layui-card-body" style="padding: 10px;background:rgb(240,230,221);">
            <form class="layui-form" action="" lay-filter="component-form-group">
                                ...
                                <div class="layui-form-item">
                                    ...
                                    <div class="layui-input-block">
                                        <select name="option_id" lay-verify="required" lay-filter="option_id">
                                            ...
                                            @foreach($data['options'] as $ke=>$vo)
                                            <option value="{{$vo['id']}}" @if($vo['id']==$data['selected']['option_id'])
                                                selected
                                            @endif data-remark="{{$vo['content']}}">{{$vo['name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="layui-colla-item">
                                            ...
                                            <div class="layui-colla-content" style="max-height: 200px;overflow-y: auto;overflow-x:hidden;">
                                                        <span class="rightRemark f15">
                                                            @foreach($data['options'] as $ke=>$vo)
                                                                @if($vo['id']==$data['selected']['option_id'])
                                                                    {{$vo['content']}}
                                                                @endif
                                                            @endforeach
                                                        </span>
                ...
                <div class="disf" style="justify-content: center;">
                    <a href="/?customer/discuss_online&id={{$id}}" style="background:#05af05;color:{{$website['color_word']}};padding:7px 18px;border-radius: 2px;box-sizing: border-box;margin-right:10px;white-space: nowrap;text-align: center;">我要讨论</a>
                    @if($data['is_ending']==0)
...
<script>
    layui.config({
        ...
        form.on('select(select_enterprise)',function(data){
            let val = data.value;
            if(val != ''){
                $.post('/member/join_group',{'group_id':"{{$data['group_id']}}",'status':val},function(res){
