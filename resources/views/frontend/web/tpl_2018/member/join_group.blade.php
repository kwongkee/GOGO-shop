                ...
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 30px 0;border:0;">
                    ...
                    <div class="layui-colla-item">
                        <h2 class="layui-colla-title2 disf">
                            ...
                            <div class="arrow_div @if($vo['status']==1)
                                    r_green2
                                    @elseif($vo['status']==0)
                                    r_blue
                                    @elseif($vo['status']==-1)
...
<script>
        ...
        if(now_status==0){
            ...
            if(status==-1){
                layer.confirm('确认拒绝加入该群组？', {
                    ...
                    $.post('/member/join_group',{'group_id':id,'status':status},function(res){
    ...
    function resolution(id){
        ...
        var index = layer.open({
            type: 2,
            title: '查看议题',
            content: '/member/topics_detail&id=' + id + '&is_edit=<?php echo base64_encode(1);?>',
            area: ['100%', '100%']
        });
    }
</script>