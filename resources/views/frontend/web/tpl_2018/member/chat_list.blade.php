...
<style type="text/css">
    ...
    /**状态颜色END**/
    .layui-colla-item{border: 5px solid {{$website['color']}};}
    ...
    .layui-colla-title2 .title{width: 17%;text-align: center;margin-right: 3%;border-right: 1px solid {{$website['color_word']}};font-weight:800;}
    .layui-colla-title2 .sel{width: 62%;padding-right: 3%;}
    .layui-colla-title2 .sel .layui-anim-upbit{width: 100%;}
    .layui-colla-icon{right: 0px;width: 15%;position: relative;left: 0;text-align: center;}
    .arrow_div{width: 15%;height:100%;text-align: center;display: flex;align-items: center;justify-content: center;border-left:1px solid {{$website['color_word']}};}
    ...
    <div class="layui-card">
        <div class="layui-card-body" style="padding: 10px 10px;">
            <form class="layui-form" action="" lay-filter="component-form-group">
                ...
                @if($vo['id']==237)
                <!--订单-->
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 20px;box-shadow: 0px 0px 10px 0px #999;">
                    <div class="layui-colla-item">
                        <h2 class="layui-colla-title2 disf">
                            ...
                            <div class="sel">
                                <select name="orderid" id="orderid">
                                    ...
                                    <option value="{{$vo2['id']}}">{{$vo2['ordersn']}}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="layui-colla-content layui-show">
                            <div class="txt_btn" onclick="chat_list(1,{{$vo['id']}})">讨论</div>
                            <div class="txt_btn" onclick="open_list(1,{{$vo['id']}})">查看</div>
                        </div>
                    </div>
                </div>
                @elseif($vo['id']==238)
                ...
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 20px;box-shadow: 0px 0px 10px 0px #999;">
                    <div class="layui-colla-item">
                        <h2 class="layui-colla-title2 disf">
                            ...
                            <div class="sel">
                                <select name="gid" id="gid">
                                    ...
                                    <option value="">暂无数据</option>
                                    @else
                                    @foreach($vo['list'] as $key=>$vo2)
                                    <option value="{{$vo2['goods_id']}}">{{$vo2['goods_name']}}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="layui-colla-content">
                            <div class="txt_btn" onclick="chat_list(2,{{$vo['id']}})">讨论</div>
                            <div class="txt_btn" onclick="open_list(2,{{$vo['id']}})">查看</div>
                        </div>
                    </div>
                </div>
                @elseif($vo['id']==239)
                ...
                <div class="layui-collapse" lay-accordion="now" style="margin:10px 0 20px;box-shadow: 0px 0px 10px 0px #999;">
                    <div class="layui-colla-item">
                        <h2 class="layui-colla-title2 disf">
                            <div class="title">{{$vo['title']}}</div>
                            <div class="sel">
                                <select name="topics_id" id="topics_id">
                                    @if(empty($vo['list']))
                                    <option value="">暂无数据</option>
                                    @else
                                    @foreach($vo['list'] as $key=>$vo2)
                        ...
                        <div class="layui-colla-content">
                            <div class="txt_btn" onclick="chat_list(3,{{$vo['id']}})">讨论</div>
                            <div class="txt_btn" onclick="open_list(3,{{$vo['id']}})">查看</div>
                        </div>
                    </div>
                </div>
                @endif
...
<script>
    ...
    function chat_list(type,pid){
        var url = '';
        if(type==1){
            let id = $('#orderid').val();
            if(id!=''){
                url = '/customer/customer_online&pid='+pid+'&id='+id;
            }
        }
        else if(type==2){
            let id = $('#gid').val();
            ...
            let id = $('#topics_id').val();
            if(id!=''){
                url = '/customer/discuss_online&pid='+pid+'&id='+id;
            }
        }
    ...
    function open_list(type,pid){
        var layer = layui.layer;

        if(type==1){
            let id = $('#orderid').val();
            if(id!=''){
                parent.location.href = 'https://www.gogo198.cn/cart/cart_detail?id='+id;
            }
        }
        else if(type==2){
            let id = $('#gid').val();
            if(id!='') {
                parent.location.href = 'https://www.gogo198.cn/goods-' + id + '.html';
            }
        }
        else if(type==3){
            let id = $('#topics_id').val();
            if(id!='') {
                let url = '/member/topics_detail&id=' + id + '&is_edit=<?php echo base64_encode(1);?>';
                var index = layer.open({
                    type: 2,
                    title: '打开议题',
                    content: url,
