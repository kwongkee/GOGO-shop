            ...
            <div class="layui-card">
                <div class="layui-card-body" style="background:#fff;padding:10px;">
                    <div class="layui-btn-group test-table-operate-btn" style="margin-bottom: 10px;">
                        <button class="layui-btn layui-btn-normal" onclick="openWindow('添加一级菜单','/func/site/save_footer');">添加一级菜单</button>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#mainTable'
            ,url: "/func/site/footer_manage?pa=1"
            ,cellMinWidth: 200
            ,cols: [[
                ...
                ,{width:180, align:'center', fixed: 'right', title: '操作', templet: function(d){
                        ...
                        return [
                            '<a onclick="openWindow('+ "'["+d.name+"]下级列表'" +',' + "'/func/site/footer_child?pid="+ d.id + '\');" class="layui-btn layui-btn-xs layui-btn-primary">下级列表</a>',
                            '<a onclick="openWindow('+ "'["+d.name+"]编辑'" +',' + "'/func/site/save_footer?&id="+ d.id + '\');" class="layui-btn layui-btn-xs layui-btn-normal">编辑</a>',
    ...
    function del(id){
        ...
        layer.confirm('确认要删除该菜单（包含下级）吗？',{
            btn:['删除','取消']
        }, function(){
            $.ajax({
                url:"/func/site/del_footer",
                method:'post',
