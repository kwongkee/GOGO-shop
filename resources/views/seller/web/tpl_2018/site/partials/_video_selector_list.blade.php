...
{{--                !poster.png?x-oss-process=image/resize,m_pad,limit_0,h_120,w_120--}}
            <li class="image-item" title="H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10 640x368 24fps 425.3642578125Kbps" data-id="{{ $v->video_id }}">
                <video class="image-box" src="{{ get_image_url($v->path) }}" data-path="{{ $v->path }}" data-url="{{ get_image_url($v->path) }}" data-poster="{{ get_image_url($v->path) }}!poster.png" data-width="{{ $v->width }}" data-height="{{ $v->height }}" style="width: 100%;height:100%;"/>
