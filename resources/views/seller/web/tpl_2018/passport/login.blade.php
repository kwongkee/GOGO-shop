...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="https://{{ env('FRONTEND_DOMAIN') }}/css/common.css">
    <script src="/assets/d2eace91/js/jquery.js?v=20180418"></script>
    <script type="text/javascript" src="https://{{ env('FRONTEND_DOMAIN') }}/js/common.js"></script>
...
<div class="header login-header w990">
    ...
    <div class="logo-right" style="display: none;">
        <ul class="logo-right-menu">
            <li class="current"><a class="seller" href="https://{{ env('SELLER_DOMAIN') }}/index.html" target="_blank"><i></i>商家管理中心</a></li>
            <li><a class="store" href="https://{{ env('STORE_DOMAIN') }}/index.html" target="_blank"><i></i>网点管理中心</a></li>
        </ul>
    </div>
</div>
...
<div class="login-content">
    <div class="login-banner" style="background: url('{{ get_image_url(sysconf('login_bg_image')) }}') center;">
        <div class="w990 pos-r">
            ...
            <link type="text/css" rel="stylesheet" href="https://{{ env('FRONTEND_DOMAIN') }}/css/login.css">
            <link rel="stylesheet" href="/assets/d2eace91/iconfont/iconfont.css?v=1.6"/>
            <!--整站改色 _start-->
            @if(sysconf('custom_style_enable') == 1)
                <link rel="stylesheet" href="https://{{ env('FRONTEND_DOMAIN') }}/css/custom/site-color-style-0.css?v=1.6"/>
            @else
                <link rel="stylesheet" href="https://{{ env('FRONTEND_DOMAIN') }}/css/color-style.css?v=1.6"/>
            @endif
            <!--整站改色 _end-->
            ...
            <div id="1524381447rads9d" class="login-form">
                <div class="login-con pos-r">
                    ...
                    <div class="login-switch" style="display: none;">
                        <a href="javascript:;" class="qrcode-target btn-qrcode" on-moblie="false" title="手机扫码登录"></a>
                    </div>

                    <div class="login-wrap ">

                        <div class="login-tit" style="display: none;">
                            还不是卖家？
                            <a href="https://{{ env('FRONTEND_DOMAIN') }}/shop/apply.html" class="regist-link color">
                                立即申请
                                <i>&gt;</i>
                            </a>
                        </div>
...
{{--                                <li class="active" id="login2" onclick="setTab('login',2,2)">普通登录</li>--}}
                        ...
                        @if(!empty($auto_login))
                            <div id="con_login_2" class="form" style="display: none;">
                                <form id="form2" action="/login" method="POST">
                                    ...
                                    <div class="form-group item-name">
                                        ...
                                        <div class="form-control-box">
                                            <i class="icon"></i>
                                            <input type="text" id="username" name="LoginModel[username]" value="{{$auto_login['mobile']}}" class="text" tabindex="1" placeholder="已验证手机/邮箱/用户名" autocomplete="off" />
                                        </div>
                                    ...
                                    <div class="form-group item-password">
                                        <div class="form-control-box">
                                            <i class="icon"></i>
                                            <input type="password" id="password" name="LoginModel[password]" value="{{$auto_login['password']}}" class="text" tabindex="2" placeholder="密码" autocomplete="off" />
                                        </div>
...
{{--                                        <a class="forget-password fr" href="https://{{ env('FRONTEND_DOMAIN') }}/user/find-password">忘记密码？</a>--}}
                                    </div>
                                    <div class="login-btn">
                                        <input type="hidden" name="act" value="act_login" />
                                        <input type="hidden" name="back_act" value="" />
                                        <input type="submit" name="submit" class="btn-img btn-entry bg-color lkdl" value="立即登录" />
                                    </div>
...
{{--                                    <input type="hidden" name="back_url" value=" {{$_SERVER['HTTP_REFERER'] ?? '' }}" />--}}
                            ...
                            <div id="con_login_2" class="form" style="display: none;">
                                <form id="form2" action="/login" method="POST">
                                    ...
                                    <div class="form-group item-name">
                                        ...
                                        <div class="form-control-box">
                                            <i class="icon"></i>
                                            <input type="text" id="username" name="LoginModel[username]" value="" class="text" tabindex="1" placeholder="已注册的手机号码" autocomplete="off" />
                                        </div>
    ...
    {{--                                <input type="hidden" name="back_url" value=" {{$_SERVER['HTTP_REFERER'] ?? '' }}" />--}}
                            ...
                            <div id="con_login_1" class="form" style="display: block;">
            ...
            <script type="text/javascript">
                $().ready(function() {
                    ...
                    $("body").on('click', '.website-login', function() {
                        ...
                        $.go("https://{{ env('FRONTEND_DOMAIN') }}"+"/website/login?type="+type);
                    })
                ...
                function REF_QRCodeLogin(view){
                    $.get('/site/get-qrcode-login-key',{ },function(result){
                        ...
                        $('.SZY-QRCODE-LOGIN').qrcode({
                            width:150,
                            height:150,
                            text: 'https://{{ env('FRONTEND_DOMAIN') }}/site/qrcode-login.html?k=' + result.data.key
                        });
